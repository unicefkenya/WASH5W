...
import { WorkflowStatusState } from '../models/workflow-status-state.model';

const LOG_PREFIX: string = "[Workflow Statuses Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class WorkflowStatusesDataService {

  // Keeps tabs of the default url string
  private url: string = environment.production ?
    `${environment.urls.api}/api/v1/workflow_statuses` :
    `${environment.urls.api}/workflow_statuses`;

  // Keeps tabs of the user defined search or sort criteria.
  private state: WorkflowStatusState = {
    searchTerm: null,
    page: 1,
    pageSize: 20,
    sortColumn: 'name',
  ...
  // Keeps tabs of the workflowStatuses
  private workflowStatusesSubject$ = new BehaviorSubject<WorkflowStatus[]>([]);
  readonly workflowStatuses$ = this.workflowStatusesSubject$.asObservable();

  // Keeps tabs of the total workflowStatuses (irregardless of pagination)
  ...
  /**
   * Creates a new Workflow Status Record
   * 
   * @param workflowStatus the details of the Workflow Status Record to be created
   * @returns the newly created Workflow Status Record
   */
  public createWorkflowStatus(workflowStatus: WorkflowStatus): Observable<WorkflowStatus> {

    this.log.trace(`${LOG_PREFIX} Entering createWorkflowStatus()`);
    this.log.debug(`${LOG_PREFIX} Workflow Status = ${JSON.stringify(workflowStatus)}`);
    ...
    return this.http.post<WorkflowStatus>(`${this.url}`, JSON.stringify(workflowStatus), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: WorkflowStatus) => {

          // Workflow Status Record Creation was successful
          ...
          this.getFreshWorkflowStatuses(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Workflow Status Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Status Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Retrieves Workflow Statuses Records from the local cache or database
   * 
   ...
   * @returns The Workflow Statuses Records
   */
  public getWorkflowStatuses(cache: boolean, state: WorkflowStatusState): Observable<WorkflowStatus[]> {

    this.log.trace(`${LOG_PREFIX} Entering getWorkflowStatuses()`);
    ...
    if (state) {
      ...
      if (JSON.stringify(this.state) === JSON.stringify(state)) {
        ...
        if (this.workflowStatusesSubject$.value.length > 0) {
          ...
          return of(this.workflowStatusesSubject$.value);
    ...
    // Get a fresh set of Workflow Statuses Records from the backend
    this.log.trace(`${LOG_PREFIX} Getting a fresh set of Workflow Statuses Records from the backend`);
    return this.getFreshWorkflowStatuses(cache, state);

  }


  /**
   * Retrieves a fresh set of Workflow Statuses Records from the backend
   * 
   * @param cache Whether the results should be locally cached if freshly retrieved from the database
   * @param state The desired records state
   * @returns The Workflow Statuses Records
   */  
  private getFreshWorkflowStatuses(cache: boolean, state: WorkflowStatusState): Observable<WorkflowStatus[]> {

    this.log.trace(`${LOG_PREFIX} Entering getFreshWorkflowStatuses()`);
    ...
    return this.http.get<WorkflowStatus[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: parameters }), observe: 'response' })
      .pipe(

        map((res: HttpResponse<any>) => {

          // Workflow Statuses Records Retrieval was successful
          this.log.trace(`${LOG_PREFIX} Workflow Statuses Records Retrieval was successful`);
          ...
          if (cache) {
            ...
            this.workflowStatusesSubject$.next(res.body);
        ...
        catchError((error: any) => {
          ...
          // Workflow Statuses Records Retrieval was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Statuses Records Retrieval was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Workflow Status Record
   * 
   * @param workflowStatus The details of the Workflow Status Record to be updated
   * @returns the updated Workflow Status Record
   */
  public updateWorkflowStatus(workflowStatus: WorkflowStatus): Observable<WorkflowStatus> {

    this.log.trace(`${LOG_PREFIX} Entering updateWorkflowStatus()`);
    this.log.debug(`${LOG_PREFIX} Workflow Status = ${JSON.stringify(workflowStatus)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${workflowStatus.id} to update the record`);
    return this.http.put<WorkflowStatus>(`${this.url}/${workflowStatus.id} `, JSON.stringify(workflowStatus), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: WorkflowStatus) => {

          // Workflow Status Record Update was successful
          this.log.trace(`${LOG_PREFIX} Workflow Status Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Workflow Status Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Status Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Workflow Status Record
   *
   * @param workflowStatusId The id of the Workflow Status Record to be deleted
   */
  public deleteWorkflowStatus(workflowStatusId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteWorkflowStatus()`);
    this.log.debug(`${LOG_PREFIX} Workflow Status Id = ${workflowStatusId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${workflowStatusId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${workflowStatusId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Workflow Status Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Workflow Status Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Workflow Status Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Status Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
   ...
   */
  public get records(): WorkflowStatus[] {
    return this.workflowStatusesSubject$.value;
  }
   ...
   */
  private getQueryParams(state: WorkflowStatusState): any {
