...
const LOG_PREFIX: string = "[Logical Elements Types Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class LogicalElementsTypesSelectionDataService {
  ...
  private getFreshLogicalElementsTypes(cache: boolean, state: LogicalElementTypeState): Observable<LogicalElementType[]> {
    ...
    return this.http.get<LogicalElementType[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
