...
const LOG_PREFIX: string = "[Assigned Administrative Systems Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class AssignedAdministrativeSystemsSelectionDataService {
  ...
  private getFreshAdministrativeSystems(cache: boolean, state: AdministrativeSystemState): Observable<AdministrativeSystem[]> {
    ...
    return this.http.get<AdministrativeSystem[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
