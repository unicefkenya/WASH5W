...
export class QuantitiesObservationsDataService {
    ...
    return this.http.post<QuantityObservation>(`${this.url}`, JSON.stringify(quantityObservation), { headers: new HttpHeaders(HEADERS) })
      .pipe(
        ...
        tap((data: QuantityObservation) => {
          ...
          this.getFreshQuantitiesObservations(true, this.state).subscribe();
   ...
   */
  public getQuantitiesObservations(cache: boolean, state: QuantityObservationState): Observable<QuantityObservation[]> {

    this.log.trace(`${LOG_PREFIX} Entering getQuantitiesObservations()`);
    ...
    return this.getFreshQuantitiesObservations(cache, state);
   ...
   * @returns The Administrative Units Records
   */
  private getFreshQuantitiesObservations(cache: boolean, state: QuantityObservationState): Observable<QuantityObservation[]> {

    this.log.trace(`${LOG_PREFIX} Entering getFreshQuantitiesObservations()`);
