...
const LOG_PREFIX: string = "[Data Forms Groups Records Updation Component]";

@Component({
  selector: 'sb-data-forms-groups-records-updation',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './data-forms-groups-records-updation.component.html',
  styleUrls: ['data-forms-groups-records-updation.component.scss'],
})
export class DataFormsGroupsRecordsUpdationComponent implements OnInit, OnDestroy {
  ...
  public dataFormGroup: DataFormElement | null | undefined;
  ...
  ngOnInit() {
    ...
    this.initialiseDataFormGroup(() => {

      this.initialiseRelevancyRule(() => {

        this.initialiseRepeatabilityRule(() => {

          this.initialiseDataFormsGroupsForm(() => {

            this.initialiseDataFormsGroupsFormChangesListener(() => {
  ...
  /**
   * Retrieves the Data Form Group with the injected id and sets it as the Data Form Group that needs to be updated
   * @param callback The function to call when done
   */
  private initialiseDataFormGroup(callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseDataFormGroup()`);
    this.log.debug(`${LOG_PREFIX} Target Id = ${this.id}`);

    // Get the data form group corresponding to the passed in id
    this.log.trace(`${LOG_PREFIX} Getting the data form group corresponding to the passed in id`);
    this.getDataFormGroup$(this.id)
      .subscribe({
        next: (dataFormGroup: DataFormElement | null) => {
  ...
  /**
   * Creates a local copy of the relevancy rule fashioned after the previously saved details in the target Data Form Group
   * @param callback The function to call when done
   */
  private initialiseRelevancyRule(callback: () => void): void {
    ...
    this.relevancyRule = this.dataFormGroup ? Object.assign({}, this.dataFormGroup?.data.conditionalRelevancyRule) : null;
  ...
  /**
   * Creates a local copy of the repeatability rule fashioned after the previously saved details in the target Data Form Group
   * @param callback The function to call when done
   */
  private initialiseRepeatabilityRule(callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseRepeatabilityRule()`);

    // Initialise the repeatability rule
    this.log.trace(`${LOG_PREFIX} Initialise the repeatability rule`);
    this.repeatabilityRule = this.dataFormGroup ? Object.assign({}, this.dataFormGroup?.data.repeatabilityRule) : null;
  ...
  /**
  * Initialises the data group updation reactive form
  * @param callback The function to call once the initialisation is complete 
  */
  private initialiseDataFormsGroupsForm(callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseDataFormsGroupsForm()`);

    // Update the form
    this.log.trace(`${LOG_PREFIX} Updating the form`);
    this.dataFormsGroupsForm.get('titled')?.setValue(this.dataFormGroup?.data.titled ? this.dataFormGroup?.data.titled : false);
    this.dataFormsGroupsForm.get('title')?.setValue(this.dataFormGroup?.data.title ? this.dataFormGroup?.data.title : null);
    this.dataFormsGroupsForm.get('described')?.setValue(this.dataFormGroup?.data.described ? this.dataFormGroup?.data.described : false);
    this.dataFormsGroupsForm.get('description')?.setValue(this.dataFormGroup?.data.description ? this.dataFormGroup?.data.description : null);
    this.dataFormsGroupsForm.get('conditionallyRelevant')?.setValue(this.dataFormGroup?.data.conditionallyRelevant ? this.dataFormGroup?.data.conditionallyRelevant : false);
    this.dataFormsGroupsForm.get('repeated')?.setValue(this.dataFormGroup?.data.repeated ? this.dataFormGroup?.data.repeated : false);
    this.dataFormsGroupsForm.get('layoutId')?.setValue(this.dataFormGroup?.data.layoutId? this.dataFormGroup?.data.layoutId : 1);
   ...
   */
  private initialiseDataFormsGroupsFormChangesListener(callback: (() => void)): void {
  ...
  private broadcastPageType(callback: (() => void)): void {
    ...
    if (this.isConditional() || this.isRepeated()) {
  ...
  /**
  * Retrieves a Data Form Group record given its unique identifier synchronously
  * @param id The unique identifier of the Data Form Group
  */
  public getDataFormGroup$(id: number | null | undefined): Observable<DataFormElement | null> {

    this.log.trace(`${LOG_PREFIX} Entering getDataFormGroup$()`);

    // Check if the Data Form Group's Id was specified
    this.log.trace(`${LOG_PREFIX} Checking if the Data Form Group's Id was specified`);
    if (id) {

      // The Data Form Group's Id was specified
      this.log.trace(`${LOG_PREFIX} The Data Form Group's Id was specified`);
      this.log.debug(`${LOG_PREFIX} Data Form Group = ${JSON.stringify(id)}`);

      // Asynchronously get and return the Data Form Group
      return new Observable(obs => {
        ...
        if (dataFormGroup) {
          ...
          this.log.debug(`${LOG_PREFIX} Data Form Group Record = ${JSON.stringify(dataFormGroup)}`);

          // Return the Data Form Group
          this.log.trace(`${LOG_PREFIX} Returning the Data Form Group`);
          obs.next(dataFormGroup);
      ...
      // The Data Form Group's Id was not specified
      this.log.trace(`${LOG_PREFIX} The Data Form Group's Id was not specified`);
    ...
    if (this.isValid()) {
      ...
      this.dataFormsElementsDataService
        .updateDataFormElement(
          new DataFormElement(
            {
              id: this.dataFormGroup?.id,
              data: {
                contextId: this.contextId,
                dataFormId: this.dataFormId,
                categoryId: 1,
                typeId: this.isRepeated() ? 2 : 1,
                parentId: this.dataFormGroup?.data.parentId,
                layoutId: this.getLayoutId(),
                index: this.dataFormGroup?.data.index,
                code: null,
              ...
              version: this.dataFormGroup?.version
            }))
        .subscribe({
