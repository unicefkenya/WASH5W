...
const LOG_PREFIX: string = "[Workflow Transitions Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class WorkflowTransitionsSelectionDataService {
  ...
  private getFreshWorkflowTransitions(cache: boolean, state: WorkflowTransitionState): Observable<WorkflowTransition[]> {
    ...
    return this.http.get<WorkflowTransition[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
