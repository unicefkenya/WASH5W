...
import { WorkflowTransitionsRecordsCreationModalComponent } from '@modules/workflow-transitions/containers/workflow-transitions-records-creation-modal/workflow-transitions-records-creation-modal.component';
import { WorkflowTransitionsRecordsDeletionModalComponent } from '@modules/workflow-transitions/containers/workflow-transitions-records-deletion-modal/workflow-transitions-records-deletion-modal.component';
import { WorkflowTransitionsRecordsUpdationModalComponent } from '@modules/workflow-transitions/containers/workflow-transitions-records-updation-modal/workflow-transitions-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Workflow Transitions Records Tabulation Component]";

@Component({
    selector: 'sb-workflow-transitions-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './workflow-transitions-records-tabulation.component.html',
    styleUrls: ['workflow-transitions-records-tabulation.component.scss'],
})
export class WorkflowTransitionsRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public onWorkflowChange(): void {
        ...
        if (workflowId != this.stateSubject$.value.fromStateId) {
            ...
            // Update the global filter
            this.log.trace(`${LOG_PREFIX} Updating the global filter`);
            this.filterService.update({ activeWorkflow: this.workflowsDataService.records.find(a => a.id == workflowId) });
    ...
    /**
     * Handles Workflow Transitions Records Addition Requests
     */
    public onAddWorkflowTransition(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddWorkflowTransition()`);
        const modalRef = this.modalService.open(WorkflowTransitionsRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.workflowId = this.stateSubject$.value.fromStateId;
    }

    /**
     * Handles Workflow Transitions Records Updation Requests
     * @param id The unique identifier of the Workflow Transition record to update
     */
    public onUpdateWorkflowTransition(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateWorkflowTransition()`);
        this.log.debug(`${LOG_PREFIX} Workflow Transition Record Id = ${id}`);
        const modalRef = this.modalService.open(WorkflowTransitionsRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Workflow Transitions Records Deletion Requests
     * * @param id The unique identifier of the Workflow Transition record to delete
     */
    public onDeleteWorkflowTransition(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteWorkflowTransition()`);
        this.log.debug(`${LOG_PREFIX} Workflow Transition Record Id = ${id}`);
        const modalRef = this.modalService.open(WorkflowTransitionsRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
