...
const LOG_PREFIX: string = "[Logical Hierarchies Records Tabulation Component]";

@Component({
    selector: 'sb-logical-hierarchies-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './logical-hierarchies-records-tabulation.component.html',
    styleUrls: ['logical-hierarchies-records-tabulation.component.scss'],
})
export class LogicalHierarchiesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.initialiseActiveContext(() => {
            ...
            this.initialiseActiveContextChangesHandler(() => {
                ...
                // Retrieve and cache logical schemes locally
                this.initialiseLogicalSchemes(() => {

                    //  Set the active context's logical scheme as the active logical scheme
                    ...
                    next: (filter) => {
                        ...
                        if ((JSON.stringify(this.activeContext) !== JSON.stringify(filter.activeContext))) {
                            ...
                            this.initialiseActiveLogicalScheme(() => {
                                ...
                                // Re-initialise the logical structures
                                this.initialiseLogicalStructures(() => {

                                    // Clear the currently active logical structure
                                    this.filterService.filter.activeLogicalStructures.length = 0;

                                    // Re-initialise the active logical structures
                                    this.initialiseActiveLogicalStructures(() => {
                                        ...
                                        copy.contextId = filter.activeContext?.id;
                                        copy.typesIds = temp;
    ...
    private initialiseActiveLogicalScheme(callback: () => void): void {
        ...
        // Get the active context's logical scheme record
        this.log.trace(`${LOG_PREFIX} Getting the active context's logical scheme record`);
        const logicalScheme: LogicalScheme | null | undefined = (this.filterService.filter.activeContext?.data.schemeId && this.logicalSchemesDataService.records.length > 0) ? this.logicalSchemesDataService.records.find(l => l.id == this.filterService.filter.activeContext?.data.schemeId) : null;
        this.log.trace(`${LOG_PREFIX} Context's Logical Scheme record = ${JSON.stringify(logicalScheme)}`);
    ...
    /**
     * Handles Logical Hierarchies Records Addition Requests
     */
    public onAddLogicalHierarchy(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddLogicalHierarchy()`);

        // Retrieve the last opened Logical Hierarchy
        this.log.trace(`${LOG_PREFIX} Retrieving the last opened Logical Hierarchy`);
        const activeLogicalHierarchy: LogicalHierarchy | null = this.getLastOpenedLogicalHierarchyRecord();

        // Pop up the hierarchical level addition window
        this.log.trace(`${LOG_PREFIX} Popping up the hierarchical level addition window`);
        const modalRef = this.modalService.open(LogicalHierarchiesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.contextId = this.stateSubject$.value.contextId;
        modalRef.componentInstance.scheme = this.filterService.filter.activeLogicalScheme;
        modalRef.componentInstance.commissioner = this.getCommissioner(activeLogicalHierarchy);
        modalRef.componentInstance.structures = activeLogicalHierarchy ? this.getSubsidiaryLogicalStructures(activeLogicalHierarchy.data.type?.id) :
            this.logicalStructuresDataService.records.filter(a => a.data?.commissioner?.id == null);

    }


    /**
     * Handles Logical Hierarchies Records Deletion Requests
     * @param id The unique identifier of the Logical Hierarchy record to delete
     */
    public onDeleteLogicalHierarchy(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteLogicalHierarchy()`);
        this.log.debug(`${LOG_PREFIX} Logical Hierarchy Record Id = ${id}`);
        const modalRef = this.modalService.open(LogicalHierarchiesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
