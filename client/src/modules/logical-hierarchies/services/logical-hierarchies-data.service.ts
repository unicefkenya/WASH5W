...
const LOG_PREFIX: string = "[Logical Hierarchies Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class LogicalHierarchiesDataService {
  ...
  /**
   * Creates a new Logical Hierarchy Record
   * 
   * @param logicalHierarchy the details of the Logical Hierarchy Record to be created
   * @returns the newly created Logical Hierarchy Record
   */
  public createLogicalHierarchy(logicalHierarchy: LogicalHierarchy): Observable<LogicalHierarchy> {

    this.log.trace(`${LOG_PREFIX} Entering createLogicalHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Logical Hierarchy = ${JSON.stringify(logicalHierarchy)}`);
    ...
    return this.http.post<LogicalHierarchy>(`${this.url}`, JSON.stringify(logicalHierarchy), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: LogicalHierarchy) => {

          // Logical Hierarchy Record Creation was successful
          ...
          this.getFreshLogicalHierarchies(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Logical Hierarchy Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Hierarchy Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Logical Hierarchy Record
   * 
   * @param logicalHierarchy The details of the Logical Hierarchy Record to be updated
   * @returns the updated Logical Hierarchy Record
   */
  public updateLogicalHierarchy(logicalHierarchy: LogicalHierarchy): Observable<LogicalHierarchy> {

    this.log.trace(`${LOG_PREFIX} Entering updateLogicalHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Logical Hierarchy = ${JSON.stringify(logicalHierarchy)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${logicalHierarchy.id} to update the record`);
    return this.http.put<LogicalHierarchy>(`${this.url}/${logicalHierarchy.id} `, JSON.stringify(logicalHierarchy), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: LogicalHierarchy) => {

          // Logical Hierarchy Record Update was successful
          this.log.trace(`${LOG_PREFIX} Logical Hierarchy Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Logical Hierarchy Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Hierarchy Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Logical Hierarchy Record
   *
   * @param logicalHierarchyId The id of the Logical Hierarchy Record to be deleted
   */
  public deleteLogicalHierarchy(logicalHierarchyId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteLogicalHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Logical Hierarchy Id = ${logicalHierarchyId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${logicalHierarchyId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${logicalHierarchyId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Logical Hierarchy Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Logical Hierarchy Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Logical Hierarchy Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Hierarchy Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
