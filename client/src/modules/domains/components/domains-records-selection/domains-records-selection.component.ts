...
import { DomainState } from '@modules/domains/models/domain-state.model';
import { DomainsSelectionDataService } from '@modules/domains/services/domains-selection-data.service';

const LOG_PREFIX: string = "[Domains Records Selection Component]";

@Component({
    selector: 'sb-domains-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './domains-records-selection.component.html',
    styleUrls: ['domains-records-selection.component.scss'],
})
export class DomainsRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private stateSubject$ = new BehaviorSubject<DomainState>({
        searchTerm: null,
        page: 1,
        pageSize: 10,
        sortColumn: 'name',
    ...
    constructor(
        ...
        public domainsDataService: DomainsSelectionDataService,
    ...
    private initialiseDesiredRecordsStateChangesHandler(callback: () => void): void {
        ...
        // Subscribe to desired record states changes and react to them after a little delay
            ...
            this.stateSubject$
                ...
                .subscribe({
                    next: (state) => {
                        ...
                        // Retrieve the Domain records corresponding to the passed in state
                        this.log.trace(`${LOG_PREFIX} Retrieving the Domain records corresponding to the passed in state`);
                        this.domainsDataService
                            .getDomains(true,state)
                            .pipe(first())
                            .subscribe();
    ...
    private initialiseLoadingStatusChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.domainsDataService.loading$
                .subscribe(
                    (loading) => {
    ...
    private initialiseTotalRecordCountsChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.domainsDataService.totalRecords$.subscribe(
                (total) => {
    ...
    public onSearchTermChange(searchTerm: string): void {
        ...
        if (searchTerm != this.stateSubject$.value.searchTerm) {
            ...
            let copy: DomainState = Object.assign({}, this.stateSubject$.value);
    ...
    */
    public onSelect(domain: Domain): void {
    ...
    /** 
    * Handles Domains Checkboxes Check Events
    * @param domain The Checked Domain
    */
    public onCheck(domain: Domain): void {
    ...
    /** 
    * Handles Domains Checkboxes Uncheck Events
    * @param domain The Unchecked Domain
    */
    public onUncheck(domain: Domain): void {
    ...
    /**
     * Checks whether a Domain record is currently selected
     * @param domain The target Domain
     * @returns True or false depending on whether the Domain is currently selected or not respectively
     */
    public isSelected(domain: Domain): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target Domain = ${JSON.stringify(domain)}`);

        // Check whether the Domain is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the Domain is currently selected`);
        const selected: boolean = this.selected.some(id => id == domain.id);
    ...
    /**
     * Checks whether a Domain record is currently checked
     * @param domain The target Domain
     * @returns True or false depending on whether the Domain is currently checked or not respectively
     */
    public isChecked(domain: Domain): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target Domain = ${JSON.stringify(domain)}`);

        // Check whether the Domain is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the Domain is currently checked`);
        const checked: boolean = this.selected.some(id => id == domain.id);
    ...
    /**
     * Checks whether a Domain record is desired
     * @param domain The target Domain
     * @returns True or false depending on whether the Domain is desired or not respectively
     */
    public isDesired(domain: Domain): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target Domain = ${JSON.stringify(domain)}`);

        // Check whether the Domain is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the Domain is currently desired`);
        const desired: boolean = this.desired.some(id => id == domain.id);
    ...
    /**
     * Checks whether a Domain record is undesired
     * @param domain The target Domain
     * @returns True or false depending on whether the Domain is undesired or not respectively
     */
    public isUndesired(domain: Domain): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target Domain = ${JSON.stringify(domain)}`);

        // Check whether the Domain is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the Domain is currently undesired`);
        const undesired: boolean = this.undesired.some(id => id == domain.id);
