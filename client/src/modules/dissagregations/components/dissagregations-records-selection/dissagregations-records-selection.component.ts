...
import { Dissagregation, DissagregationState } from '@modules/dissagregations/models';
import { DissagregationType } from '@modules/dissagregations-types/models/dissagregation-type.model';

const LOG_PREFIX: string = "[Dissagregations Records Selection Component]";

@Component({
    selector: 'sb-dissagregations-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './dissagregations-records-selection.component.html',
    styleUrls: ['dissagregations-records-selection.component.scss'],
})
export class DissagregationsRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private initialiseDesiredRecordsState(callback: () => void): void {
        ...
        if (this.initialised) {
            ...
            // Check if the desired dissagregation type has been specified
            this.log.trace(`${LOG_PREFIX} Checking if the desired dissagregation type has been specified`);
            if (this.desiredType) {

                // The desired dissagregation type has been specified
                this.log.trace(`${LOG_PREFIX} The desired dissagregation type has been specified`);
                this.log.debug(`${LOG_PREFIX} Desired Dissagregation Type = ${JSON.stringify(this.desiredType)}`);

                // Initialise the dissagregation type to the specified dissagregation type
                this.log.trace(`${LOG_PREFIX} Initialising the dissagregation type to the specified dissagregation type`);
                ...
                // Initialise the dissagregation type to the default dissagregation type
                this.log.trace(`${LOG_PREFIX} Initialising the dissagregation type to the default dissagregation type`);
                Object.assign(copy, { typeId: activeDissagregationType && activeDissagregationType.id ? activeDissagregationType.id : null });
            }
        }
    ...
    /** 
    * Handles Dissagregation Selection Events
    ...
    */
    ...
    /** 
    * Handles Dissagregations Checkboxes Check Events
    ...
    */
    ...
    /** 
    * Handles Dissagregations Checkboxes Uncheck Events
    ...
    */
    ...
    /**
     * Checks whether an Dissagregation record is currently selected
     * @param dissagregation The target Dissagregation
     * @returns True or false depending on whether the Dissagregation is currently selected or not respectively
     */
    isSelected(dissagregation: Dissagregation): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target Dissagregation = ${JSON.stringify(dissagregation)}`);

        // Check whether the Dissagregation is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the Dissagregation is currently selected`);
        const selected: boolean = this.selected.some(id => id == dissagregation.id);
    ...
    /**
     * Checks whether an Dissagregation record is currently checked
     * @param dissagregation The target Dissagregation
     * @returns True or false depending on whether the Dissagregation is currently checked or not respectively
     */
    isChecked(dissagregation: Dissagregation): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target Dissagregation = ${JSON.stringify(dissagregation)}`);

        // Check whether the Dissagregation is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the Dissagregation is currently checked`);
        const checked: boolean = this.selected.some(id => id == dissagregation.id);
    ...
    /**
     * Checks whether an Dissagregation record is desired
     * @param dissagregation The target Dissagregation
     * @returns True or false depending on whether the Dissagregation is desired or not respectively
     */
    isDesired(dissagregation: Dissagregation): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target Dissagregation = ${JSON.stringify(dissagregation)}`);

        // Check whether the Dissagregation is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the Dissagregation is currently desired`);
        const desired: boolean = this.desired.some(id => id == dissagregation.id);
    ...
    /**
     * Checks whether an Dissagregation record is undesired
     * @param dissagregation The target Dissagregation
     * @returns True or false depending on whether the Dissagregation is undesired or not respectively
     */
    isUndesired(dissagregation: Dissagregation): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target Dissagregation = ${JSON.stringify(dissagregation)}`);

        // Check whether the Dissagregation is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the Dissagregation is currently undesired`);
        const undesired: boolean = this.undesired.some(id => id == dissagregation.id);
