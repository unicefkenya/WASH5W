...
const LOG_PREFIX: string = "[Systems Roles Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class SystemsRolesDataService {
  ...
  /**
   * Creates a new System Role Record
   * 
   * @param systemRole the details of the System Role Record to be created
   * @returns the newly created System Role Record
   */
  public createSystemRole(systemRole: SystemRole): Observable<SystemRole> {

    this.log.trace(`${LOG_PREFIX} Entering createSystemRole()`);
    ...
    return this.http.post<SystemRole>(`${this.url}`, JSON.stringify(systemRole), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: SystemRole) => {

          // System Role Record Creation was successful
          ...
          this.getFreshSystemsRoles(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // System Role Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} System Role Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single System Role Record
   * 
   * @param systemRole The details of the System Role Record to be updated
   * @returns the updated System Role Record
   */
  public updateSystemRole(systemRole: SystemRole): Observable<SystemRole> {

    this.log.trace(`${LOG_PREFIX} Entering updateSystemRole()`);
    this.log.debug(`${LOG_PREFIX} System Role = ${JSON.stringify(systemRole)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${systemRole.id} to update the record`);
    return this.http.put<SystemRole>(`${this.url}/${systemRole.id} `, JSON.stringify(systemRole), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: SystemRole) => {

          // System Role Record Update was successful
          this.log.trace(`${LOG_PREFIX} System Role Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // System Role Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} System Role Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single System Role Record
   *
   * @param systemRoleId The id of the System Role Record to be deleted
   */
  public deleteSystemRole(systemRoleId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteSystemRole()`);
    this.log.debug(`${LOG_PREFIX} System Role Id = ${systemRoleId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${systemRoleId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${systemRoleId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // System Role Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} System Role Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // System Role Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} System Role Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
