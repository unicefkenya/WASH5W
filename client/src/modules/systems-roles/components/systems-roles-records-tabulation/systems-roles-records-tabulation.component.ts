...
import { SystemsRolesRecordsCreationModalComponent } from '@modules/systems-roles/containers/systems-roles-records-creation-modal/systems-roles-records-creation-modal.component';
import { SystemsRolesRecordsDeletionModalComponent } from '@modules/systems-roles/containers/systems-roles-records-deletion-modal/systems-roles-records-deletion-modal.component';
import { SystemsRolesRecordsUpdationModalComponent } from '@modules/systems-roles/containers/systems-roles-records-updation-modal/systems-roles-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Systems Roles Records Tabulation Component]";

@Component({
    selector: 'sb-systemsRoles-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './systems-roles-records-tabulation.component.html',
    styleUrls: ['systems-roles-records-tabulation.component.scss'],
})
export class SystemsRolesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Handles Systems Roles Records Addition Requests
     */
    public onAddSystemRole(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddSystemRole()`);

        this.modalService.open(SystemsRolesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
    }

    /**
     * Handles Systems Roles Records Updation Requests
     */
    public onUpdateSystemRole(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateSystemRole()`);
        this.log.debug(`${LOG_PREFIX} System Role Id = ${id}`);

        const modalRef = this.modalService.open(SystemsRolesRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Systems Roles Records Deletion Requests
     */
    public onDeleteSystemRole(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteSystemRole()`);
        this.log.debug(`${LOG_PREFIX} System Role Id = ${id}`);

        const modalRef = this.modalService.open(SystemsRolesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
