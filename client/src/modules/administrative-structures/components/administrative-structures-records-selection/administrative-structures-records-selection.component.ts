...
import { AdministrativeStructure, AdministrativeStructureState } from '@modules/administrative-structures/models';
...
const LOG_PREFIX: string = "[Administrative Structures Records Selection Component]";

@Component({
    selector: 'sb-administrative-structures-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './administrative-structures-records-selection.component.html',
    styleUrls: ['administrative-structures-records-selection.component.scss'],
})
export class AdministrativeStructuresRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {

    // Allows the parent component to inject the desired selection mode: single or multi
    @Input() public selectionMode: string = "single";

    // Allows the parent component to inject the desired Administrative Structure
    @Input() public desired: AdministrativeStructure[] = [];    

    // Allows the parent component to inject the undesired Administrative Structures
    // Ignored if the desired Administrative Structures has been specified
    @Input() public undesired: AdministrativeStructure[] = [];

    // Allows the parent component to inject the previously selected Administrative Structures
    ...
    /** 
    * Handles Administrative Structure Selection Events
    ...
    */
    ...
    /** 
    * Handles Administrative Structures Checkboxes Check Events
    ...
    */
    ...
    /** 
    * Handles Administrative Structures Checkboxes Uncheck Events
    ...
    */
    ...
    /**
     * Checks whether an Administrative Structure record is currently selected
     * @param administrativeStructure The target Administrative Structure
     * @returns True or false depending on whether the Administrative Structure is currently selected or not respectively
     */
    isSelected(administrativeStructure: AdministrativeStructure): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target Administrative Structure = ${JSON.stringify(administrativeStructure)}`);

        // Check whether the Administrative Structure is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the Administrative Structure is currently selected`);
        const selected: boolean = this.selected.some(a => a.id == administrativeStructure.id);
    ...
    /**
     * Checks whether an Administrative Structure record is currently checked
     * @param administrativeStructure The target Administrative Structure
     * @returns True or false depending on whether the Administrative Structure is currently checked or not respectively
     */    
    isChecked(administrativeStructure: AdministrativeStructure): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target Administrative Structure = ${JSON.stringify(administrativeStructure)}`);

        // Check whether the Administrative Structure is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the Administrative Structure is currently checked`);
        const checked: boolean = this.selected.some(a => a.id == administrativeStructure.id);
    ...
    /**
     * Checks whether an Administrative Structure record is desired
     * @param administrativeStructure The target Administrative Structure
     * @returns True or false depending on whether the Administrative Structure is desired or not respectively
     */
     isDesired(administrativeStructure: AdministrativeStructure): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target Administrative Structure = ${JSON.stringify(administrativeStructure)}`);

        // Check whether the Administrative Structure is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the Administrative Structure is currently desired`);
        const desired: boolean = this.desired.some(a => a.id == administrativeStructure.id);
    ...
    /**
     * Checks whether an Administrative Structure record is undesired
     * @param administrativeStructure The target Administrative Structure
     * @returns True or false depending on whether the Administrative Structure is undesired or not respectively
     */
     isUndesired(administrativeStructure: AdministrativeStructure): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target Administrative Structure = ${JSON.stringify(administrativeStructure)}`);

        // Check whether the Administrative Structure is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the Administrative Structure is currently undesired`);
        const undesired: boolean = this.undesired.some(a => a.id == administrativeStructure.id);
