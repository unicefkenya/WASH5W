...
const LOG_PREFIX: string = "[Assigned Administrative Structures Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class AssignedAdministrativeStructuresSelectionDataService {
  ...
  private getFreshAdministrativeStructures(cache: boolean, state: AdministrativeStructureState): Observable<AdministrativeStructure[]> {
    ...
    return this.http.get<AdministrativeStructure[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
