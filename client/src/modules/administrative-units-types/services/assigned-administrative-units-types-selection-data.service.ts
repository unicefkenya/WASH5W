...
const LOG_PREFIX: string = "[Assigned Administrative Units Types Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class AssignedAdministrativeUnitsTypesSelectionDataService {
  ...
  private getFreshAdministrativeUnitsTypes(cache: boolean, state: AdministrativeUnitTypeState): Observable<AdministrativeUnitType[]> {
    ...
    return this.http.get<AdministrativeUnitType[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
