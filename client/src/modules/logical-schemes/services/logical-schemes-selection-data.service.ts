...
const LOG_PREFIX: string = "[Logical Schemes Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class LogicalSchemesSelectionDataService {
  ...
  private getFreshLogicalSchemes(cache: boolean, state: LogicalSchemeState): Observable<LogicalScheme[]> {
    ...
    return this.http.get<LogicalScheme[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
