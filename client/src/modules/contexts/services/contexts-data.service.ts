...
const LOG_PREFIX: string = "[Contexts Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class ContextsDataService {
   ...
   * 
   * @param context the details of the Context Record to be created
   * @returns the newly created Context Record
   */
  public createContext(context: Context): Observable<Context> {

    this.log.trace(`${LOG_PREFIX} Entering createContext()`);
    ...
    return this.http.post<Context>(`${this.url}`, JSON.stringify(context), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Context) => {

          // Context Record Creation was successful
          ...
          this.getFreshContexts(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Context Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Context Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Context Record
   * 
   * @param context The details of the Context Record to be updated
   * @returns the updated Context Record
   */
  public updateContext(context: Context): Observable<Context> {

    this.log.trace(`${LOG_PREFIX} Entering updateContext()`);
    this.log.debug(`${LOG_PREFIX} Context = ${JSON.stringify(context)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${context.id} to update the record`);
    return this.http.put<Context>(`${this.url}/${context.id} `, JSON.stringify(context), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Context) => {

          // Context Record Update was successful
          this.log.trace(`${LOG_PREFIX} Context Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Context Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Context Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Context Record
   *
   * @param contextId The id of the Context Record to be deleted
   */
  public deleteContext(contextId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteContext()`);
    this.log.debug(`${LOG_PREFIX} Context Id = ${contextId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${contextId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${contextId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Context Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Context Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Context Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Context Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
