...
const LOG_PREFIX: string = "[Indicators Records Updation Component]";

@Component({
  selector: 'sb-indicators-records-updation',
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './indicators-records-updation.component.html',
  styleUrls: ['indicators-records-updation.component.scss'],
})
export class IndicatorsRecordsUpdationComponent implements OnInit {
  ...
  // Holds the target Indicator record
  public indicator: Indicator | null | undefined = undefined;

  // Keep tabs on other context indicators
  public contextIndicators: Indicator[] = [];
  ...
  ngOnInit() {
    ...
    this.initialiseIndicator(() => {
      ...
      // Initialise the target form field if the indicator is form field fed
      this.initialiseFormField(this.indicator?.data.formFieldId, () => {

        // Initialise the target subindicators if the indicator is subindicator fed
        this.initialiseSubindicatorsRecords(this.indicator?.data.subindicatorsIds, () => {

          // Preselect the active Context in the data tabulation form
          this.initialiseFormGroup(() => {

            // Mark the current page as the first page if it is filled via subindicators or form field results
            if (this.indicator?.data.subindicatorsFilled || this.indicator?.data?.formFilled) {
  ...
  /**
 * Retrieves the Indicator with the injected id and sets it as the Indicator that needs to be updated
 * @param callback The function to call when done
 */
  private initialiseIndicator(callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseIndicator()`);
  ...
  /**
   * Retrieves and caches Indicators records
   * @param callback The function to call when done
   */
  private initialiseContextIndicators(callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseContextIndicators()`);

    // Retrieve and cache the context's Indicators records
    this.log.trace(`${LOG_PREFIX} Retrieving and caching the context's Indicators records`);
    this.indicatorsDataService
      .getIndicators(true, {
        ...
        sortDirection: 'asc',
        ids: null,
        contextId: this.indicator?.data.contextId,
        no: null,
        name: null,
      ...
      .subscribe({
        next: (indicators: Indicator[]) => {
          ...
          // Indicators successfully retrieved and cached
          this.log.debug(`${LOG_PREFIX} ${indicators.length} Indicator(s) retrieved and cached`);
        ...
        error: (err: any) => {
          ...
          this.log.error(`${LOG_PREFIX} Indicators retrieval failed`);
  ...
  /**
   * Initialises the form field field given its unique identifier
   * @param id The unique identifier of the Data Form Element
   * @param callback The function to call when done
   */
  private initialiseFormField(id: number | null | undefined, callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseFormField()`);
        ...
        .subscribe({
          next: (dataFormElements: DataFormElement[]) => {
            ...
            if (dataFormElements.length > 0) {
              ...
              this.log.warn(`${LOG_PREFIX} Initialising form field`);
              this.formField = dataFormElements[0];
              ...
              // Initialise form field to null
              this.log.warn(`${LOG_PREFIX} Initialising form field to null`);
              this.formField = null;
  ...
  /**
   * Retrieves a subindicators records given their unique identifiers synchronously
   * @param ids The unique identifiers of the subindicators
   * @param callback The function to call when done
   */
  private initialiseSubindicatorsRecords(ids: number[] | null | undefined, callback: () => void): void {

    this.log.trace(`${LOG_PREFIX} Entering initialiseSubindicatorsRecords()`);
 ...
 */
  private initialiseFormGroup(callback: () => void): void {
    ...
    this.retrieveLogicalElementRecord(this.indicator?.data.logicalParentId, (logicalParent: LogicalElement | null) => {
      this.retrieveUnitRecord(this.indicator?.data.unitId, (unit: Unit | null) => {

        this.defaultForm.get('logicalElement.logicalElementId')?.setValue(logicalParent?.id ? logicalParent.id : null);
        ...
        this.defaultForm.get('logicalElement.truncatedLogicalElementName')?.setValue(logicalParent?.data.name ? this.truncate(logicalParent.data.name) : this.truncate("Optionally choose logical parent"));
        this.defaultForm.get('no')?.setValue(this.indicator?.data?.no ? this.indicator.data.no : null);
        this.defaultForm.get('name')?.setValue(this.indicator?.data?.name ? this.indicator.data.name : null);
        this.defaultForm.get('unit.unitId')?.setValue(unit?.id ? unit.id : null);
        this.defaultForm.get('unit.unitAbbreviation')?.setValue(unit?.data.abbreviation ? unit.data.abbreviation : "");
        this.defaultForm.get('subindicatorsFilled')?.setValue(this.indicator?.data?.subindicatorsFilled ? this.indicator.data.subindicatorsFilled : false);
        this.defaultForm.get('formFilled')?.setValue(this.indicator?.data?.formFilled ? this.indicator.data.formFilled : false);
        this.defaultForm.get('cumulative')?.setValue(this.indicator?.data?.cumulative ? this.indicator.data.cumulative : false);
        this.rulesForm.get('aggregationId')?.setValue(this.indicator?.data?.autoFillingRule?.aggregationId ? this.indicator?.data?.autoFillingRule?.aggregationId : null);
        this.rulesForm.get('optionId')?.setValue(this.indicator?.data?.autoFillingRule?.optionId ? this.indicator?.data?.autoFillingRule?.optionId : null);
  ...
  */
  ...
  /**
  * Retrieves a Data Form Element Record given its unique identifier synchronously
  * @param id The unique identifier of the DataFormElement
  * @param callback The function to call when done
  */
  private retrieveDataFormElementRecord(id: number | null | undefined, callback: (entity: DataFormElement | null) => void): void {
      ...
      this.dataFormsElementsDataService
        ...
        .subscribe({
          next: (dataFormElements: DataFormElement[]) => {
            ...
            // Check if a Data Form Element Record with the given id was found
            this.log.trace(`${LOG_PREFIX} Checking if a Data Form Element Record with the given id was found`);
            if (dataFormElements.length > 0) {

              //A Data Form Element Record with the given id was found
              this.log.trace(`${LOG_PREFIX} A Data Form Element Record with the given id was found`);

              // Return the Data Form Element Record
              this.log.trace(`${LOG_PREFIX} Returning the Data Form Element Record`);
              callback(dataFormElements[0]);


            } else {

              //A Data Form Element Record with the given id was not found
              this.log.trace(`${LOG_PREFIX} A Data Form Element Record with the given id was not found`);
   ...
   * @param callback The function to call when done
   */
  private retrieveUnitRecord(id: number | null | undefined, callback: (entity: Unit | null) => void): void {
      ...
      this.unitsDataService
        ...
        .subscribe({
          next: (units: Unit[]) => {
            ...
            // Check if a Unit record with the given id was found
            this.log.trace(`${LOG_PREFIX} Checking if a Unit record with the given id was found`);
            if (units.length > 0) {

              //An Unit record with the given id was found
              this.log.trace(`${LOG_PREFIX} An Unit record with the given id was found`);

              // Return the Unit record
              this.log.trace(`${LOG_PREFIX} Returning the Unit record`);
              callback(units[0]);


            } else {

              //An Unit record with the given id was not found
              this.log.trace(`${LOG_PREFIX} An Unit record with the given id was not found`);
  ...
  public getContextId(): number | null | undefined {
    return this.indicator?.data.contextId
  }
  ...
  /**
   * Retrieves the id and name of the selected subindicators
   * @returns the ids
   */
  public getSelectedSubindicatorsDetails(): { id: number | null | undefined; name: string | null | undefined; }[] {

    this.log.trace(`${LOG_PREFIX} Entering getSelectedSubindicatorsDetails()`);

    const details: { id: number | null | undefined; name: string | null | undefined; }[] = [];
    for (let s of this.subindicators) {
      if (s) {
        details.push({ id: s.id, name: s.data.name });
      }
    }

    this.log.debug(`${LOG_PREFIX} Selected Subindicators Details = ${JSON.stringify(details)}`);
  ...
  */
            ...
            .pipe(
              map((indicators: Indicator[]) => {
                ...
                if (indicators.length > 0) {
                  ...
                  // Check if the indicator is different from the current indicator
                  this.log.trace(`${LOG_PREFIX} Checking if the indicator is different from the current indicator`);
                  for (let indicator of indicators) {
                    if (indicator.id != this.indicator?.id) {

                      // The indicator is different from the current indicator
                      this.log.trace(`${LOG_PREFIX} The indicator is different from the current indicator`);
                  ...
                  // The indicator is not different from the current indicator
                  this.log.trace(`${LOG_PREFIX} The indicator is not different from the current indicator`);
  ...
  private isAggregationSpecificationRequired(): ValidatorFn {
    ...
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (this.subindicatorsFilled() || this.formFilled()) {
    ...
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      if (this.subindicatorsFilled() || this.formFilled()) {
        ...
        if (control.value) {
          ...
          if (this.subindicatorsFilled() || this.formField?.data.typeId == 3 || this.formField?.data.typeId == 4) {
  ...
  private isSubindicatorsSpecificationValid(): boolean {
    ...
    if (this.defaultForm && this.subindicatorsFilled()) {
  ...
  private isFormFieldSpecificationValid(): boolean {
    ...
    if (this.defaultForm && this.formFilled()) {
          ...
          if (this.rulesForm.valid) {
            ...
            this.indicatorsDataService
              .updateIndicator(new Indicator({
                ...
                version: this.indicator?.version
              }))
              .subscribe({
                next: (response: Indicator) => {
                  ...
                  this.defaultForm.get('no')?.setValue('');
                  this.defaultForm.get('name')?.setValue('');
                  this.defaultForm.get('unit.unitId')?.setValue(null);
                  this.defaultForm.get('unit.unitAbbreviation')?.setValue(null);
                  this.defaultForm.get('subindicatorsFilled')?.setValue(false);
                  this.defaultForm.get('formFilled')?.setValue(false);
                  this.rulesForm.get('aggregationId')?.setValue(null);
                  this.rulesForm.get('optionId')?.setValue(null);
