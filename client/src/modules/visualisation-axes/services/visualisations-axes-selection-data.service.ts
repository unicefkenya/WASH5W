...
const LOG_PREFIX: string = "[Visualisation Axes Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class VisualisationsAxesSelectionDataService {
  // Keeps tabs of the default url string
  private url: string = environment.production ?
    `${environment.urls.api}/api/v1/visualisation_axes` :
    `${environment.urls.api}/visualisation_axes`;
   ...
   */
   private getQueryParams(state: VisualisationAxisState): any {
    ...
    if (state.id) {
      ...
      params['id']=state.visualisationId;
    }    
