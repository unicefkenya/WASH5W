...
const LOG_PREFIX: string = "[Units Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class UnitsSelectionDataService {
  ...
  private getFreshUnits(cache: boolean, state: UnitState): Observable<Unit[]> {
    ...
    return this.http.get<Unit[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
