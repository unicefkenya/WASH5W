...
import { SystemsUsersSelectionDataService } from '@modules/systems-users/services/systems-users-selection-data.service';

const LOG_PREFIX: string = "[Systems Users Records Selection Component]";

@Component({
    selector: 'sb-systems-users-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './systems-users-records-selection.component.html',
    styleUrls: ['systems-users-records-selection.component.scss'],
})
export class SystemsUsersRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        public systemsUsersDataService: SystemsUsersSelectionDataService,
    ...
    /** 
    * Handles SystemUser Selection Events
    * @param systemUser The Selected SystemUser
    */
    public onSelect(systemUser: SystemUser): void {
    ...
    /** 
    * Handles Systems Users Checkboxes Check Events
    * @param systemUser The Checked SystemUser
    */
    public onCheck(systemUser: SystemUser): void {
    ...
    /** 
    * Handles Systems Users Checkboxes Uncheck Events
    * @param systemUser The Unchecked SystemUser
    */
    public onUncheck(systemUser: SystemUser): void {
    ...
    /**
     * Checks whether an System User record is currently selected
     * @param systemUser The target System User
     * @returns True or false depending on whether the System User is currently selected or not respectively
     */
    public isSelected(systemUser: SystemUser): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target System User = ${JSON.stringify(systemUser)}`);

        // Check whether the System User is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the System User is currently selected`);
        const selected: boolean = this.selected.some(id => id == systemUser.id);
    ...
    /**
     * Checks whether an System User record is currently checked
     * @param systemUser The target System User
     * @returns True or false depending on whether the System User is currently checked or not respectively
     */
    public isChecked(systemUser: SystemUser): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target System User = ${JSON.stringify(systemUser)}`);

        // Check whether the System User is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the System User is currently checked`);
        const checked: boolean = this.selected.some(id => id == systemUser.id);
    ...
    /**
     * Checks whether an System User record is desired
     * @param systemUser The target System User
     * @returns True or false depending on whether the System User is desired or not respectively
     */
    public isDesired(systemUser: SystemUser): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target System User = ${JSON.stringify(systemUser)}`);

        // Check whether the System User is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the System User is currently desired`);
        const desired: boolean = this.desired.some(id => id == systemUser.id);
    ...
    /**
     * Checks whether an System User record is undesired
     * @param systemUser The target System User
     * @returns True or false depending on whether the System User is undesired or not respectively
     */
    public isUndesired(systemUser: SystemUser): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target System User = ${JSON.stringify(systemUser)}`);

        // Check whether the System User is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the System User is currently undesired`);
        const undesired: boolean = this.undesired.some(id => id == systemUser.id);
