...
const LOG_PREFIX: string = "[Systems Users Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class SystemsUsersDataService {
   ...
   * 
   * @param systemUser the details of the System User Record to be created
   * @returns the newly created System User Record
   */
  public createSystemUser(systemUser: SystemUser): Observable<SystemUser> {

    this.log.trace(`${LOG_PREFIX} Entering createSystemUser()`);
    ...
    this.log.trace(`${LOG_PREFIX} Checking if we are in a development environment`);
    if (!environment.production) {

      // We are in a development environment
      this.log.trace(`${LOG_PREFIX} We are in a development environment`);

      // Encode the password
      this.log.trace(`${LOG_PREFIX} Encoding the password`);
      systemUser = Object.assign(systemUser, { data: Object.assign(systemUser.data, { password: window.btoa(systemUser.data.password as string) }) });

    } else {

      // We are not in a development environment
      this.log.trace(`${LOG_PREFIX} We are not in a development environment`);

      // Delegate password encoding to the backend
      this.log.trace(`${LOG_PREFIX} Delegating password encoding to the backend`);
    ...
    return this.http.post<SystemUser>(`${this.url}`, JSON.stringify(systemUser), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: SystemUser) => {

          // System User Record Creation was successful
          ...
          this.getFreshSystemsUsers(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // System User Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} System User Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
    ...
    return this.http.get<SystemUser[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: this.getQueryParams(state), observe: 'response' })
      .pipe(
        ...
        catchError((error: any) => {
          ...
          throw new Error("Systems Users Retrieval Failed");
  ...
  /**
   * Updates a single System User Record
   * 
   * @param systemUser The details of the System User Record to be updated
   * @returns the updated System User Record
   */
  public updateSystemUser(systemUser: SystemUser): Observable<SystemUser> {

    this.log.trace(`${LOG_PREFIX} Entering updateSystemUser()`);
    this.log.debug(`${LOG_PREFIX} System User = ${JSON.stringify(systemUser)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${systemUser.id} to update the record`);
    return this.http.put<SystemUser>(`${this.url}/${systemUser.id} `, JSON.stringify(systemUser), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: SystemUser) => {

          // System User Record Update was successful
          this.log.trace(`${LOG_PREFIX} System User Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // System User Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} System User Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single System User Record
   *
   * @param systemUserId The id of the System User Record to be deleted
   */
  public deleteSystemUser(systemUserId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteSystemUser()`);
    this.log.debug(`${LOG_PREFIX} System User Id = ${systemUserId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${systemUserId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${systemUserId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // System User Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} System User Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // System User Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} System User Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
