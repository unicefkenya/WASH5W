...
const LOG_PREFIX: string = "[Visualisations Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class VisualisationsDataService {
  ...
  /**
   * Creates a new Visualisation Record
   * 
   * @param visualisation the details of the Visualisation Record to be created
   * @returns the newly created Visualisation Record
   */
  public createVisualisation(visualisation: Visualisation): Observable<Visualisation> {

    this.log.trace(`${LOG_PREFIX} Entering createVisualisation()`);
    this.log.debug(`${LOG_PREFIX} Visualisation = ${JSON.stringify(visualisation)}`);
    ...
    return this.http.post<Visualisation>(`${this.url}`, JSON.stringify(visualisation), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Visualisation) => {

          // Visualisation Record Creation was successful
          ...
          this.getFreshVisualisations(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Visualisation Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Visualisation Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  private getFreshVisualisations(cache: boolean, state: VisualisationState): Observable<Visualisation[]> {
    ...
    return this.http.get<Visualisation[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: parameters }), observe: 'response' })
      .pipe(
  ...
  /**
   * Updates a single Visualisation Record
   * 
   * @param visualisation The details of the Visualisation Record to be updated
   * @returns the updated Visualisation Record
   */
  public updateVisualisation(visualisation: Visualisation): Observable<Visualisation> {

    this.log.trace(`${LOG_PREFIX} Entering updateVisualisation()`);
    this.log.debug(`${LOG_PREFIX} Visualisation = ${JSON.stringify(visualisation)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${visualisation.id} to update the record`);
    return this.http.put<Visualisation>(`${this.url}/${visualisation.id} `, JSON.stringify(visualisation), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Visualisation) => {

          // Visualisation Record Update was successful
          this.log.trace(`${LOG_PREFIX} Visualisation Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Visualisation Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Visualisation Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Visualisation Record
   *
   * @param visualisationId The id of the Visualisation Record to be deleted
   */
  public deleteVisualisation(visualisationId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteVisualisation()`);
    this.log.debug(`${LOG_PREFIX} Visualisation Id = ${visualisationId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${visualisationId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${visualisationId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Visualisation Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Visualisation Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Visualisation Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Visualisation Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
