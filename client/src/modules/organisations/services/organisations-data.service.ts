...
const LOG_PREFIX: string = "[Organisations Data Service]";
...
export class OrganisationsDataService {
   ...
   * 
   * @param organisation the details of the Organisation Record to be created
   * @returns the newly created Organisation Record
   */
  public createOrganisation(organisation: Organisation): Observable<Organisation> {

    this.log.trace(`${LOG_PREFIX} Entering createOrganisation()`);
    ...
    return this.http.post<Organisation>(`${this.url}`, JSON.stringify(organisation), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Organisation) => {

          // Organisation Record Creation was successful
          ...
          this.getFreshOrganisations(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Organisation Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Organisation Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Organisation Record
   * 
   * @param organisation The details of the Organisation Record to be updated
   * @returns the updated Organisation Record
   */
  public updateOrganisation(organisation: Organisation): Observable<Organisation> {

    this.log.trace(`${LOG_PREFIX} Entering updateOrganisation()`);
    this.log.debug(`${LOG_PREFIX} Organisation = ${JSON.stringify(organisation)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${organisation.id} to update the record`);
    return this.http.put<Organisation>(`${this.url}/${organisation.id} `, JSON.stringify(organisation), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Organisation) => {

          // Organisation Record Update was successful
          this.log.trace(`${LOG_PREFIX} Organisation Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Organisation Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Organisation Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Organisation Record
   *
   * @param organisationId The id of the Organisation Record to be deleted
   */
  public deleteOrganisation(organisationId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteOrganisation()`);
    this.log.debug(`${LOG_PREFIX} Organisation Id = ${organisationId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${organisationId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${organisationId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Organisation Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Organisation Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Organisation Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Organisation Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
