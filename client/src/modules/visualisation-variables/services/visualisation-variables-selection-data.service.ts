...
const LOG_PREFIX: string = "[Visualisation Variables Selection Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class VisualisationVariablesSelectionDataService {
  ...
  /**
   * Retrieves Visualisation Variables Records from the local cache or database
   * 
   ...
   */
  public getVisualisationVariables(cache: boolean, state: VisualisationVariableState): Observable<VisualisationVariable[]> {

    this.log.trace(`${LOG_PREFIX} Entering getVisualisationVariables()`);
    ...
    return this.getFreshVisualisationVariables(cache, state);
   ...
   * @returns The Visualisation Variables Records
   */
  private getFreshVisualisationVariables(cache: boolean, state: VisualisationVariableState): Observable<VisualisationVariable[]> {

    this.log.trace(`${LOG_PREFIX} Entering getFreshVisualisationVariables()`);
    ...
    return this.http.get<VisualisationVariable[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: this.getQueryParams(state) }), observe: 'response' })
      .pipe(
