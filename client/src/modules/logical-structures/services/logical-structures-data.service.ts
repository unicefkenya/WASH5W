...
import { LogicalStructure } from '../models/logical-structure.model';
import { environment } from 'environments/environment';
import { catchError, map, tap } from 'rxjs/operators';
import { LogicalStructureState } from '../models/logical-structure-state.model';

const LOG_PREFIX: string = "[Logical Structures Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class LogicalStructuresDataService {

  // Keeps tabs of the default url string
  private url: string = environment.production ?
    `${environment.urls.api}/api/v1/logical_structures` :
    `${environment.urls.api}/logical_structures`;

  // Keeps tabs of the user defined search or sort criteria.
  private state: LogicalStructureState = {
    page: 1,
    pageSize: 20,
    searchTerm: null,
    sortColumn: 'id',
  ...
  // Keeps tabs of the logicalStructures
  private logicalStructuresSubject$ = new BehaviorSubject<LogicalStructure[]>([]);
  readonly logicalStructures$ = this.logicalStructuresSubject$.asObservable();

  // Keeps tabs of the total logicalStructures (irregardless of pagination)
  ...
  /**
   * Creates a new Logical Structure Record
   * 
   * @param logicalStructure the details of the Logical Structure Record to be created
   * @returns the newly created Logical Structure Record
   */
  public createLogicalStructure(logicalStructure: LogicalStructure): Observable<LogicalStructure> {

    this.log.trace(`${LOG_PREFIX} Entering createLogicalStructure()`);
    this.log.debug(`${LOG_PREFIX} Logical Structure = ${JSON.stringify(logicalStructure)}`);
    ...
    return this.http.post<LogicalStructure>(`${this.url}`, JSON.stringify(logicalStructure), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: LogicalStructure) => {

          // Logical Structure Record Creation was successful
          ...
          this.getFreshLogicalStructures(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Logical Structure Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Structure Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Retrieves Logical Structures Records from the local cache or database
   * 
   ...
   * @returns The Logical Structures Records
   */
  public getLogicalStructures(cache: boolean, state: LogicalStructureState): Observable<LogicalStructure[]> {

    this.log.trace(`${LOG_PREFIX} Entering getLogicalStructures()`);
    ...
    if (state) {
      ...
      if (JSON.stringify(this.state) === JSON.stringify(state)) {
        ...
        if (this.logicalStructuresSubject$.value.length > 0) {
          ...
          return of(this.logicalStructuresSubject$.value);
    ...
    // Get a fresh set of Logical Structures Records from the backend
    this.log.trace(`${LOG_PREFIX} Getting a fresh set of Logical Structures Records from the backend`);
    return this.getFreshLogicalStructures(cache, state);

  }


  /**
   * Retrieves a fresh set of Logical Structures Records from the backend
   * 
   * @param cache Whether the results should be locally cached if freshly retrieved from the database
   * @param state The desired records state
   * @returns The Logical Structures Records
   */
  private getFreshLogicalStructures(cache: boolean, state: LogicalStructureState): Observable<LogicalStructure[]> {

    this.log.trace(`${LOG_PREFIX} Entering getFreshLogicalStructures()`);
    ...
    return this.http.get<LogicalStructure[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: parameters }), observe: 'response' })
      .pipe(

        map((res: HttpResponse<any>) => {

          // Logical Structures Records Retrieval was successful
          this.log.trace(`${LOG_PREFIX} Logical Structures Records Retrieval was successful`);
          ...
          if (cache) {
            ...
            this.logicalStructuresSubject$.next(res.body);
        ...
        catchError((error: any) => {
          ...
          // Logical Structures Records Retrieval was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Structures Records Retrieval was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Logical Structure Record
   * 
   * @param logicalStructure The details of the Logical Structure Record to be updated
   * @returns the updated Logical Structure Record
   */
  public updateLogicalStructure(logicalStructure: LogicalStructure): Observable<LogicalStructure> {

    this.log.trace(`${LOG_PREFIX} Entering updateLogicalStructure()`);
    this.log.debug(`${LOG_PREFIX} Logical Structure = ${JSON.stringify(logicalStructure)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${logicalStructure.id} to update the record`);
    return this.http.put<LogicalStructure>(`${this.url}/${logicalStructure.id} `, JSON.stringify(logicalStructure), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: LogicalStructure) => {

          // Logical Structure Record Update was successful
          this.log.trace(`${LOG_PREFIX} Logical Structure Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Logical Structure Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Structure Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Logical Structure Record
   *
   * @param logicalStructureId The id of the Logical Structure Record to be deleted
   */
  public deleteLogicalStructure(logicalStructureId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteLogicalStructure()`);
    this.log.debug(`${LOG_PREFIX} Logical Structure Id = ${logicalStructureId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${logicalStructureId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${logicalStructureId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Logical Structure Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Logical Structure Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Logical Structure Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Logical Structure Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
   ...
   */
  public get records(): LogicalStructure[] {
    return this.logicalStructuresSubject$.value;
  }
   ...
   */
  private getQueryParams(state: LogicalStructureState): any {
    ...
    if (state.searchTerm && state.searchTerm.trim().length > 0) {
      ...
      environment.production ? params['searchTerm'] = state.searchTerm : params['data.responsible.name_like'] = state.searchTerm;
    }
