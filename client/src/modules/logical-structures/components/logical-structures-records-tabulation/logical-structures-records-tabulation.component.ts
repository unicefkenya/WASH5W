...
import { LogicalStructuresRecordsCreationModalComponent } from '@modules/logical-structures/containers/logical-structures-records-creation-modal/logical-structures-records-creation-modal.component';
import { LogicalStructuresRecordsDeletionModalComponent } from '@modules/logical-structures/containers/logical-structures-records-deletion-modal/logical-structures-records-deletion-modal.component';
import { LogicalStructuresRecordsUpdationModalComponent } from '@modules/logical-structures/containers/logical-structures-records-updation-modal/logical-structures-records-updation-modal.component';
...
import { LogicalScheme } from '@modules/logical-schemes/models';
import { FilterService } from '@app/app-filter.service';
import { LogicalStructure } from '@modules/logical-structures/models/logical-structure.model';
import { LogicalStructureState } from '@modules/logical-structures/models/logical-structure-state.model';
...
const LOG_PREFIX: string = "[Logical Structures Records Tabulation Component]";

@Component({
    selector: 'sb-logical-structures-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './logical-structures-records-tabulation.component.html',
    styleUrls: ['logical-structures-records-tabulation.component.scss'],
})
export class LogicalStructuresRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    public onLogicalSchemeChange(): void {
        ...
        if (schemeId != this.stateSubject$.value.hierarchyId) {
            ...
            // Update the global filter
            this.log.trace(`${LOG_PREFIX} Updating the global filter`);
            this.filterService.update({ activeLogicalScheme: this.logicalSchemesDataService.records.find(a => a.id == schemeId) });
    ...
    /**
     * Handles Logical Structures Records Addition Requests
     */
    public onAddLogicalStructure(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddLogicalStructure()`);
        const modalRef = this.modalService.open(LogicalStructuresRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.schemeId = this.stateSubject$.value.hierarchyId;
    }

    /**
     * Handles Logical Structures Records Updation Requests
     * @param id The unique identifier of the Logical Structure record to update
     */
    public onUpdateLogicalStructure(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateLogicalStructure()`);
        this.log.debug(`${LOG_PREFIX} Logical Structure Record Id = ${id}`);
        const modalRef = this.modalService.open(LogicalStructuresRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Logical Structures Records Deletion Requests
     * * @param id The unique identifier of the Logical Structure record to delete
     */
    public onDeleteLogicalStructure(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteLogicalStructure()`);
        this.log.debug(`${LOG_PREFIX} Logical Structure Record Id = ${id}`);
        const modalRef = this.modalService.open(LogicalStructuresRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
