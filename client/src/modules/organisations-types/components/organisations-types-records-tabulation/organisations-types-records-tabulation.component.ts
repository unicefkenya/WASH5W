...
import { OrganisationsTypesRecordsCreationModalComponent } from '@modules/organisations-types/containers/organisations-types-records-creation-modal/organisations-types-records-creation-modal.component';
import { OrganisationsTypesRecordsDeletionModalComponent } from '@modules/organisations-types/containers/organisations-types-records-deletion-modal/organisations-types-records-deletion-modal.component';
import { OrganisationsTypesRecordsUpdationModalComponent } from '@modules/organisations-types/containers/organisations-types-records-updation-modal/organisations-types-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Organisations Types Records Tabulation Component]";

@Component({
    selector: 'sb-organisationsTypes-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './organisations-types-records-tabulation.component.html',
    styleUrls: ['organisations-types-records-tabulation.component.scss'],
})
export class OrganisationsTypesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Handles Organisations Types Records Addition Requests
     */
    public onAddOrganisationType(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddOrganisationType()`);

        this.modalService.open(OrganisationsTypesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
    }

    /**
     * Handles Organisations Types Records Updation Requests
     */
    public onUpdateOrganisationType(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateOrganisationType()`);
        this.log.debug(`${LOG_PREFIX} Organisation Type Id = ${id}`);

        const modalRef = this.modalService.open(OrganisationsTypesRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Organisations Types Records Deletion Requests
     */
    public onDeleteOrganisationType(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteOrganisationType()`);
        this.log.debug(`${LOG_PREFIX} Organisation Type Id = ${id}`);

        const modalRef = this.modalService.open(OrganisationsTypesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
