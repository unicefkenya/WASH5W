...
import { OrganisationTypeState } from '@modules/organisations-types/models/organisation-type-state.model';
import { OrganisationsTypesSelectionDataService } from '@modules/organisations-types/services/organisations-types-selection-data.service';

const LOG_PREFIX: string = "[Organisations Types Records Selection Component]";

@Component({
    selector: 'sb-organisationsTypes-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './organisations-types-records-selection.component.html',
    styleUrls: ['organisations-types-records-selection.component.scss'],
})
export class OrganisationsTypesRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private stateSubject$ = new BehaviorSubject<OrganisationTypeState>({
        searchTerm: null,
        page: 1,
        pageSize: 10,
        sortColumn: 'id',
    ...
    constructor(
        ...
        public organisationsTypesDataService: OrganisationsTypesSelectionDataService,
            ...
            this.stateSubject$
                ...
                .subscribe({
                    next: (state) => {
                        ...
                        // Retrieve the Organisation Type records corresponding to the passed in state
                        this.log.trace(`${LOG_PREFIX} Retrieving the Organisation Type records corresponding to the passed in state`);
                        this.organisationsTypesDataService
                            .getOrganisationsTypes(true,state)
                            .pipe(first())
                            .subscribe();
    ...
    private initialiseLoadingStatusChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.organisationsTypesDataService.loading$
                .subscribe(
                    (loading) => {
    ...
    private initialiseTotalRecordCountsChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.organisationsTypesDataService.totalRecords$.subscribe(
                (total) => {
    ...
    public onSearchTermChange(searchTerm: string): void {
        ...
        if (searchTerm != this.stateSubject$.value.searchTerm) {
            ...
            let copy: OrganisationTypeState = Object.assign({}, this.stateSubject$.value);
    ...
    /** 
    * Handles OrganisationType Selection Events
    * @param organisationType The Selected OrganisationType
    */
    public onSelect(organisationType: OrganisationType): void {
    ...
    /** 
    * Handles Organisations Types Checkboxes Check Events
    * @param organisationType The Checked OrganisationType
    */
    public onCheck(organisationType: OrganisationType): void {
    ...
    /** 
    * Handles Organisations Types Checkboxes Uncheck Events
    * @param organisationType The Unchecked OrganisationType
    */
    public onUncheck(organisationType: OrganisationType): void {
    ...
    /**
     * Checks whether an Organisation Type record is currently selected
     * @param organisationType The target Organisation Type
     * @returns True or false depending on whether the Organisation Type is currently selected or not respectively
     */
    public isSelected(organisationType: OrganisationType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target Organisation Type = ${JSON.stringify(organisationType)}`);

        // Check whether the Organisation Type is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the Organisation Type is currently selected`);
        const selected: boolean = this.selected.some(id => id == organisationType.id);
    ...
    /**
     * Checks whether an Organisation Type record is currently checked
     * @param organisationType The target Organisation Type
     * @returns True or false depending on whether the Organisation Type is currently checked or not respectively
     */
    public isChecked(organisationType: OrganisationType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target Organisation Type = ${JSON.stringify(organisationType)}`);

        // Check whether the Organisation Type is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the Organisation Type is currently checked`);
        const checked: boolean = this.selected.some(id => id == organisationType.id);
    ...
    /**
     * Checks whether an Organisation Type record is desired
     * @param organisationType The target Organisation Type
     * @returns True or false depending on whether the Organisation Type is desired or not respectively
     */
    public isDesired(organisationType: OrganisationType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target Organisation Type = ${JSON.stringify(organisationType)}`);

        // Check whether the Organisation Type is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the Organisation Type is currently desired`);
        const desired: boolean = this.desired.some(id => id == organisationType.id);
    ...
    /**
     * Checks whether an Organisation Type record is undesired
     * @param organisationType The target Organisation Type
     * @returns True or false depending on whether the Organisation Type is undesired or not respectively
     */
    public isUndesired(organisationType: OrganisationType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target Organisation Type = ${JSON.stringify(organisationType)}`);

        // Check whether the Organisation Type is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the Organisation Type is currently undesired`);
        const undesired: boolean = this.undesired.some(id => id == organisationType.id);
