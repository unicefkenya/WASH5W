...
import { OptionTypeState } from '@modules/options-types/models/option-type-state.model';
import { OptionsTypesSelectionDataService } from '@modules/options-types/services/options-types-selection-data.service';

const LOG_PREFIX: string = "[Options Types Records Selection Component]";

@Component({
    selector: 'sb-options-types-records-selection',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './options-types-records-selection.component.html',
    styleUrls: ['options-types-records-selection.component.scss'],
})
export class OptionsTypesRecordsSelectionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    private stateSubject$ = new BehaviorSubject<OptionTypeState>({
        searchTerm: null,
        page: 1,
        pageSize: 5,
        sortColumn: 'id',
    ...
    constructor(
        ...
        public optionsTypesDataService: OptionsTypesSelectionDataService,
            ...
            this.stateSubject$
                ...
                .subscribe({
                    next: (state) => {
                        ...
                        // Retrieve the Option Type records corresponding to the passed in state
                        this.log.trace(`${LOG_PREFIX} Retrieving the Option Type records corresponding to the passed in state`);
                        this.optionsTypesDataService
                            .getOptionsTypes(true,state)
                            .pipe(first())
                            .subscribe();
    ...
    private initialiseLoadingStatusChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.optionsTypesDataService.loading$
                .subscribe(
                    (loading) => {
    ...
    private initialiseTotalRecordCountsChangesHandler(callback: () => void): void {
        ...
        this._subscriptions.push(
            this.optionsTypesDataService.totalRecords$.subscribe(
                (total) => {
    ...
    public onSearchTermChange(searchTerm: string): void {
        ...
        if (searchTerm != this.stateSubject$.value.searchTerm) {
            ...
            let copy: OptionTypeState = Object.assign({}, this.stateSubject$.value);
    ...
    /** 
    * Handles OptionType Selection Events
    * @param optionType The Selected OptionType
    */
    public onSelect(optionType: OptionType): void {
    ...
    /** 
    * Handles Options Types Checkboxes Check Events
    * @param optionType The Checked OptionType
    */
    public onCheck(optionType: OptionType): void {
    ...
    /** 
    * Handles Options Types Checkboxes Uncheck Events
    * @param optionType The Unchecked OptionType
    */
    public onUncheck(optionType: OptionType): void {
    ...
    /**
     * Checks whether an Option Type record is currently selected
     * @param optionType The target Option Type
     * @returns True or false depending on whether the Option Type is currently selected or not respectively
     */
    public isSelected(optionType: OptionType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isSelected()`);
        this.log.debug(`${LOG_PREFIX} Target Option Type = ${JSON.stringify(optionType)}`);

        // Check whether the Option Type is currently selected
        this.log.trace(`${LOG_PREFIX} Checking whether the Option Type is currently selected`);
        const selected: boolean = this.selected.some(id => id == optionType.id);
    ...
    /**
     * Checks whether an Option Type record is currently checked
     * @param optionType The target Option Type
     * @returns True or false depending on whether the Option Type is currently checked or not respectively
     */
    public isChecked(optionType: OptionType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isChecked()`);
        this.log.debug(`${LOG_PREFIX} Target Option Type = ${JSON.stringify(optionType)}`);

        // Check whether the Option Type is currently checked
        this.log.trace(`${LOG_PREFIX} Checking whether the Option Type is currently checked`);
        const checked: boolean = this.selected.some(id => id == optionType.id);
    ...
    /**
     * Checks whether an Option Type record is desired
     * @param optionType The target Option Type
     * @returns True or false depending on whether the Option Type is desired or not respectively
     */
    public isDesired(optionType: OptionType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isDesired()`);
        this.log.debug(`${LOG_PREFIX} Target Option Type = ${JSON.stringify(optionType)}`);

        // Check whether the Option Type is currently desired
        this.log.trace(`${LOG_PREFIX} Checking whether the Option Type is currently desired`);
        const desired: boolean = this.desired.some(id => id == optionType.id);
    ...
    /**
     * Checks whether an Option Type record is undesired
     * @param optionType The target Option Type
     * @returns True or false depending on whether the Option Type is undesired or not respectively
     */
    public isUndesired(optionType: OptionType): boolean {

        this.log.trace(`${LOG_PREFIX} Entering isUndesired()`);
        this.log.debug(`${LOG_PREFIX} Target Option Type = ${JSON.stringify(optionType)}`);

        // Check whether the Option Type is currently undesired
        this.log.trace(`${LOG_PREFIX} Checking whether the Option Type is currently undesired`);
        const undesired: boolean = this.undesired.some(id => id == optionType.id);
