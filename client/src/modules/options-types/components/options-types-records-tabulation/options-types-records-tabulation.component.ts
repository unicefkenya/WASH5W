...
import { OptionsTypesRecordsCreationModalComponent } from '@modules/options-types/containers/options-types-records-creation-modal/options-types-records-creation-modal.component';
import { OptionsTypesRecordsDeletionModalComponent } from '@modules/options-types/containers/options-types-records-deletion-modal/options-types-records-deletion-modal.component';
import { OptionsTypesRecordsUpdationModalComponent } from '@modules/options-types/containers/options-types-records-updation-modal/options-types-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Options Types Records Tabulation Component]";

@Component({
    selector: 'sb-optionsTypes-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './options-types-records-tabulation.component.html',
    styleUrls: ['options-types-records-tabulation.component.scss'],
})
export class OptionsTypesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Handles Options Types Records Addition Requests
     */
    public onAddOptionType(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddOptionType()`);

        this.modalService.open(OptionsTypesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
    }

    /**
     * Handles Options Types Records Updation Requests
     */
    public onUpdateOptionType(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateOptionType()`);
        this.log.debug(`${LOG_PREFIX} Option Type Id = ${id}`);

        const modalRef = this.modalService.open(OptionsTypesRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Options Types Records Deletion Requests
     */
    public onDeleteOptionType(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteOptionType()`);
        this.log.debug(`${LOG_PREFIX} Option Type Id = ${id}`);

        const modalRef = this.modalService.open(OptionsTypesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
