...
import { SystemsModulesRecordsCreationModalComponent } from '@modules/systems-modules/containers/systems-modules-records-creation-modal/systems-modules-records-creation-modal.component';
import { SystemsModulesRecordsDeletionModalComponent } from '@modules/systems-modules/containers/systems-modules-records-deletion-modal/systems-modules-records-deletion-modal.component';
import { SystemsModulesRecordsUpdationModalComponent } from '@modules/systems-modules/containers/systems-modules-records-updation-modal/systems-modules-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Systems Modules Records Tabulation Component]";

@Component({
    selector: 'sb-systemsModules-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './systems-modules-records-tabulation.component.html',
    styleUrls: ['systems-modules-records-tabulation.component.scss'],
})
export class SystemsModulesRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Handles Systems Modules Records Addition Requests
     */
    public onAddSystemModule(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddSystemModule()`);

        this.modalService.open(SystemsModulesRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
    }

    /**
     * Handles Systems Modules Records Updation Requests
     */
    public onUpdateSystemModule(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateSystemModule()`);
        this.log.debug(`${LOG_PREFIX} System Module Id = ${id}`);

        const modalRef = this.modalService.open(SystemsModulesRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Systems Modules Records Deletion Requests
     */
    public onDeleteSystemModule(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteSystemModule()`);
        this.log.debug(`${LOG_PREFIX} System Module Id = ${id}`);

        const modalRef = this.modalService.open(SystemsModulesRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
