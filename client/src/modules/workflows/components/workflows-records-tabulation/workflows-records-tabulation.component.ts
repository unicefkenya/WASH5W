...
import { WorkflowsRecordsCreationModalComponent } from '@modules/workflows/containers/workflows-records-creation-modal/workflows-records-creation-modal.component';
import { WorkflowsRecordsDeletionModalComponent } from '@modules/workflows/containers/workflows-records-deletion-modal/workflows-records-deletion-modal.component';
import { WorkflowsRecordsUpdationModalComponent } from '@modules/workflows/containers/workflows-records-updation-modal/workflows-records-updation-modal.component';
...
const LOG_PREFIX: string = "[Workflows Records Tabulation Component]";

@Component({
    selector: 'sb-workflows-records-tabulation',
    changeDetection: ChangeDetectionStrategy.OnPush,
    templateUrl: './workflows-records-tabulation.component.html',
    styleUrls: ['workflows-records-tabulation.component.scss'],
})
export class WorkflowsRecordsTabulationComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Handles Workflows Records Addition Requests
     */
    public onAddWorkflow(): void {

        this.log.trace(`${LOG_PREFIX} Entering onAddWorkflow()`);

        this.modalService.open(WorkflowsRecordsCreationModalComponent, { centered: true, backdrop: 'static' });
    }

    /**
     * Handles Workflows Records Updation Requests
     */
    public onUpdateWorkflow(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onUpdateWorkflow()`);
        this.log.debug(`${LOG_PREFIX} Workflow Id = ${id}`);

        const modalRef = this.modalService.open(WorkflowsRecordsUpdationModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;

    }

    /**
     * Handles Workflows Records Deletion Requests
     */
    public onDeleteWorkflow(id: number): void {

        this.log.trace(`${LOG_PREFIX} Entering onDeleteWorkflow()`);
        this.log.debug(`${LOG_PREFIX} Workflow Id = ${id}`);

        const modalRef = this.modalService.open(WorkflowsRecordsDeletionModalComponent, { centered: true, backdrop: 'static' });
        modalRef.componentInstance.id = id;
    }
