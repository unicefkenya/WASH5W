...
const LOG_PREFIX: string = "[Workflows Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class WorkflowsDataService {
   ...
   * 
   * @param workflow the details of the Workflow Record to be created
   * @returns the newly created Workflow Record
   */
  public createWorkflow(workflow: Workflow): Observable<Workflow> {

    this.log.trace(`${LOG_PREFIX} Entering createWorkflow()`);
    this.log.debug(`${LOG_PREFIX} Workflow = ${JSON.stringify(workflow)}`);
    ...
    return this.http.post<Workflow>(`${this.url}`, JSON.stringify(workflow), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Workflow) => {

          // Workflow Record Creation was successful
          ...
          this.getFreshWorkflows(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Workflow Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  private getFreshWorkflows(cache: boolean, state: WorkflowState): Observable<Workflow[]> {
    ...
    return this.http.get<Workflow[]>(`${this.url}`, { headers: new HttpHeaders(HEADERS), params: new HttpParams({ fromObject: parameters }), observe: 'response' })
      .pipe(
  ...
  /**
   * Updates a single Workflow Record
   * 
   * @param workflow The details of the Workflow Record to be updated
   * @returns the updated Workflow Record
   */
  public updateWorkflow(workflow: Workflow): Observable<Workflow> {

    this.log.trace(`${LOG_PREFIX} Entering updateWorkflow()`);
    this.log.debug(`${LOG_PREFIX} Workflow = ${JSON.stringify(workflow)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${workflow.id} to update the record`);
    return this.http.put<Workflow>(`${this.url}/${workflow.id} `, JSON.stringify(workflow), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: Workflow) => {

          // Workflow Record Update was successful
          this.log.trace(`${LOG_PREFIX} Workflow Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Workflow Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Workflow Record
   *
   * @param workflowId The id of the Workflow Record to be deleted
   */
  public deleteWorkflow(workflowId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteWorkflow()`);
    this.log.debug(`${LOG_PREFIX} Workflow Id = ${workflowId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${workflowId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${workflowId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Workflow Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Workflow Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Workflow Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Workflow Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
