...
const LOG_PREFIX: string = "[Administrative Hierarchies Data Service]";
const HEADERS = { 'Content-Type': 'application/json' };

@Injectable({
  providedIn: 'root'
})
export class AdministrativeHierarchiesDataService {
  ...
  /**
   * Creates a new Administrative Hierarchy Record
   * 
   * @param administrativeHierarchy the details of the Administrative Hierarchy Record to be created
   * @returns the newly created Administrative Hierarchy Record
   */
  public createAdministrativeHierarchy(administrativeHierarchy: AdministrativeHierarchy): Observable<AdministrativeHierarchy> {

    this.log.trace(`${LOG_PREFIX} Entering createAdministrativeHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Administrative Hierarchy = ${JSON.stringify(administrativeHierarchy)}`);
    ...
    return this.http.post<AdministrativeHierarchy>(`${this.url}`, JSON.stringify(administrativeHierarchy), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: AdministrativeHierarchy) => {

          // Administrative Hierarchy Record Creation was successful
          ...
          this.getFreshAdministrativeHierarchies(true, this.state).subscribe();

        }),

        catchError((error: any) => {

          // Administrative Hierarchy Record Creation was unsuccessful
          this.log.error(`${LOG_PREFIX} Administrative Hierarchy Record Creation was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Updates a single Administrative Hierarchy Record
   * 
   * @param administrativeHierarchy The details of the Administrative Hierarchy Record to be updated
   * @returns the updated Administrative Hierarchy Record
   */
  public updateAdministrativeHierarchy(administrativeHierarchy: AdministrativeHierarchy): Observable<AdministrativeHierarchy> {

    this.log.trace(`${LOG_PREFIX} Entering updateAdministrativeHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Administrative Hierarchy = ${JSON.stringify(administrativeHierarchy)}`);
    ...
    this.log.debug(`${LOG_PREFIX} Making a HTTP POST Request to ${this.url}/${administrativeHierarchy.id} to update the record`);
    return this.http.put<AdministrativeHierarchy>(`${this.url}/${administrativeHierarchy.id} `, JSON.stringify(administrativeHierarchy), { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap((data: AdministrativeHierarchy) => {

          // Administrative Hierarchy Record Update was successful
          this.log.trace(`${LOG_PREFIX} Administrative Hierarchy Record Update was successful`);
        ...
        catchError((error: any) => {
          ...
          // Administrative Hierarchy Record Update was unsuccessful
          this.log.error(`${LOG_PREFIX} Administrative Hierarchy Record Update was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  /**
   * Deletes a single Administrative Hierarchy Record
   *
   * @param administrativeHierarchyId The id of the Administrative Hierarchy Record to be deleted
   */
  public deleteAdministrativeHierarchy(administrativeHierarchyId: number): Observable<void> {

    this.log.trace(`${LOG_PREFIX} Entering deleteAdministrativeHierarchy()`);
    this.log.debug(`${LOG_PREFIX} Administrative Hierarchy Id = ${administrativeHierarchyId}`);

    // Make a HTTP DELETE Request to delete the records
    this.log.debug(`${LOG_PREFIX} Making a HTTP DELETE Request to ${this.url}/${administrativeHierarchyId} to delete the record`);

    return this.http.delete<void>(`${this.url}/${administrativeHierarchyId}`, { headers: new HttpHeaders(HEADERS) })
      .pipe(

        tap(() => {

          // Administrative Hierarchy Record Deletion was successful
          this.log.trace(`${LOG_PREFIX} Administrative Hierarchy Record Deletion was successful`);
        ...
        catchError((error: any) => {
          ...
          // Administrative Hierarchy Record Deletion was unsuccessful
          this.log.error(`${LOG_PREFIX} Administrative Hierarchy Record Deletion was unsuccessful: ${error.statusText || "See Server Logs for more details"}`);
  ...
  private getQueryParams(state: AdministrativeHierarchyState): HttpParams {
    ...
    if (state.typesIds) {
      ...
      if(environment.production) {
        state.typesIds.forEach(id => {
          params = params.append("type.id",id);
        });
      } else {
        state.typesIds.forEach(id => {
    ...
    if (state.commissionerId) {
      ...
      environment.production ? params = params.append("commissioner.id", state.commissionerId) : params = params.append("data.commissioner.id", state.commissionerId);
    }

    if (state.commissionerName) {
      this.log.trace(`${LOG_PREFIX} Adding commissioner name parameter`);
      environment.production ? params = params.append("commissioner.name", state.commissionerName) : params = params.append("data.commissioner.name", state.commissionerName);
    }

    if (state.responsibleId) {
      this.log.trace(`${LOG_PREFIX} Adding responsible id parameter`);
      environment.production ? params = params.append("responsible.id", state.responsibleId) : params = params.append("data.responsible.id", state.responsibleId);
    }

    if (state.responsibleName) {
      this.log.trace(`${LOG_PREFIX} Adding responsible name parameter`);
      environment.production ? params = params.append("responsible.name", state.responsibleName) : params = params.append("data.responsible.name", state.responsibleName);
    }
