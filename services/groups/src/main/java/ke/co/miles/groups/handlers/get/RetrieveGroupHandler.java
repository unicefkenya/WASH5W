...
@Component
@Slf4j
public class RetrieveGroupHandler {

    @Autowired
    GroupsRepository repository;

    /**
     * Retrieves an group record given its unique identifier
     *
     * @param request the request containing the unique identifier of the group record to be retrieved and the
     *                database from which it should be retrieved
     * @return the response containing the details of the retrieved group record
     */
    public Mono<ServerResponse> retrieveGroup(ServerRequest request) {

        log.trace("Entering retrieveGroup()");
