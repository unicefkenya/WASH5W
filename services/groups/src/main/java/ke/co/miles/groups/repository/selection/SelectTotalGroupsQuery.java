...
import ke.co.miles.groups.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalGroupsQuery {
    ...
    /**
     * Selects the total number of groups records to be retrieved, or an estimate of the total number of
     * groups records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total group records should be retrieved
    ...
    public Mono<Long> selectTotalGroups(String database, MultiValueMap<String,String> parameters) {
        ...
        log.trace("Entering selectTotalGroups()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM _group" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM _group" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
