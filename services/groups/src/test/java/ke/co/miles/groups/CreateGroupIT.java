...
@ContextConfiguration(initializers = CreateGroupIT.Initializer.class)
public class CreateGroupIT {
    ...
    @Test
    public void Given_GroupDetails_When_Post_Then_GroupRecordWillBeCreatedAndReturned() {

        webTestClient
                .post()
                .uri("/api/v1/groups/test")
                .contentType(MediaType.APPLICATION_JSON)
                .body(Mono.just(group4), Group.class)
                .exchange()
                .expectStatus().isCreated()
                .expectBody(Group.class)
                .value(response -> {
                            Assertions.assertThat(response.getId()).isEqualTo(4L);
                            Assertions.assertThat(response.getData()).isEqualTo(group4.getData());
