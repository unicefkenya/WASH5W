 ...
 */
package ke.co.miles.logicalhierarchies.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.logicalhierarchies.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveLogicalHierarchiesHandler {

  @Autowired
  LogicalHierarchiesRepository repository;

  /**
   * Retrieves logicalHierarchies records
   *
   * @param request the request, optionally containing the query filters of the logicalHierarchies
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved logicalHierarchies records
   */
  public Mono<ServerResponse> retrieveLogicalHierarchies(ServerRequest request) {

    log.trace("Entering retrieveLogicalHierarchies()");

    return
        repository
            .selectTotalLogicalHierarchies(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectLogicalHierarchies(
                                request.queryParams()),
                        LogicalHierarchy.class)
                    .onErrorMap(e -> new ServerException("Logical Hierarchy deletion failed", e))
            );
  }
