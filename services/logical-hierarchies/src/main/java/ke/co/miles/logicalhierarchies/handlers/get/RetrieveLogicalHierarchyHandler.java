...
@Component
@Slf4j
public class RetrieveLogicalHierarchyHandler {

  @Autowired
  LogicalHierarchiesRepository repository;

  /**
   * Retrieves a logicalHierarchy record given its unique identifier
   *
   * @param request the request containing the unique identifier of the logicalHierarchy record to be
   *                retrieved and the database from which it should be retrieved
   * @return the response containing the details of the retrieved logicalHierarchy record
   */
  public Mono<ServerResponse> retrieveLogicalHierarchy(ServerRequest request) {

    log.trace("Entering retrieveLogicalHierarchy()");
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .selectLogicalHierarchy(Long.parseLong(request.pathVariable("id"))),
