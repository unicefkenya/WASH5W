...
import ke.co.miles.logicalhierarchies.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalLogicalHierarchiesQuery {
  ...
  /**
   * Selects the total number of logicalHierarchies records to be retrieved, or an estimate of the total
   * number of logicalHierarchies records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of logicalHierarchies records to be retrieved
   */
  public Mono<Long> selectTotalLogicalHierarchies(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalLogicalHierarchies()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM logical_hierarchy" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM logical_hierarchy" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
