...
import ke.co.miles.workflows.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveWorkflowsHandler {

  @Autowired
  WorkflowsRepository repository;

  /**
   * Retrieves workflows records
   *
   * @param request the request, optionally containing the query filters of the workflows
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved workflows records
   */
  public Mono<ServerResponse> retrieveWorkflows(ServerRequest request) {

    log.trace("Entering retrieveWorkflows()");

    return
        repository
            .selectTotalWorkflows(request.queryParams())
