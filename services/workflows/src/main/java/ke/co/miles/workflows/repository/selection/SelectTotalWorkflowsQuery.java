...
import ke.co.miles.workflows.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalWorkflowsQuery {
  ...
  /**
   * Selects the total number of workflows records to be retrieved, or an estimate of the total
   * number of workflows records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of workflows records to be retrieved
   */
  public Mono<Long> selectTotalWorkflows(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalWorkflows()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM workflow" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM workflow" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
