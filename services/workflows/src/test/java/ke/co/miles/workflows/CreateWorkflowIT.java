...
@ContextConfiguration(initializers = CreateWorkflowIT.Initializer.class)
public class CreateWorkflowIT {

  static final PostgreSQLContainer postgreSQLContainer;
  static final Workflow workflow4;
  ...
  @Test
  public void Given_WorkflowDetails_When_Post_Then_WorkflowRecordWillBeCreatedAndReturned() {

    webTestClient
        .post()
        .uri("/api/v1/workflows")
        .contentType(MediaType.APPLICATION_JSON)
        .body(Mono.just(workflow4), Workflow.class)
        .exchange()
        .expectStatus().isCreated()
        .expectBody(Workflow.class)
        .value(response -> {
              Assertions.assertThat(response.getId()).isEqualTo(4L);
              Assertions.assertThat(response.getData()).isEqualTo(workflow4.getData());
