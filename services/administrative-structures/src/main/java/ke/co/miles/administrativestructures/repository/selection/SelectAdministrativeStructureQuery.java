...
@Component
@Slf4j
public class SelectAdministrativeStructureQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects a administrativeStructure record from the database given its unique identifier
   *
   * @param id the unique identifier of the administrativeStructure record to be selected
   * @return the administrativeStructure record with the given id if found
   */
  public Mono<AdministrativeStructure> selectAdministrativeStructure(Long id) {

    log.trace("Entering selectAdministrativeStructure()");

    String query = "SELECT * FROM administrative_structure WHERE id = ?";
