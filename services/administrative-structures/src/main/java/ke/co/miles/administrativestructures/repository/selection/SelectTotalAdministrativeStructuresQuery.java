...
import ke.co.miles.administrativestructures.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAdministrativeStructuresQuery {
  ...
  /**
   * Selects the total number of administrativeStructures records to be retrieved, or an estimate of the total
   * number of administrativeStructures records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of administrativeStructures records to be retrieved
   */
  public Mono<Long> selectTotalAdministrativeStructures(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAdministrativeStructures()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM administrative_structure" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM administrative_structure" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
