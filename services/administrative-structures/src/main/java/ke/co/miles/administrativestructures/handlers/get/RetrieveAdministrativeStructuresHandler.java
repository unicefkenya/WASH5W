 ...
 */
package ke.co.miles.administrativestructures.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.administrativestructures.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveAdministrativeStructuresHandler {

  @Autowired
  AdministrativeStructuresRepository repository;

  /**
   * Retrieves administrativeStructures records
   *
   * @param request the request, optionally containing the query filters of the administrativeStructures
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved administrativeStructures records
   */
  public Mono<ServerResponse> retrieveAdministrativeStructures(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeStructures()");

    return
        repository
            .selectTotalAdministrativeStructures(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectAdministrativeStructures(
                                request.queryParams()),
                        AdministrativeStructure.class)
                    .onErrorMap(e -> new ServerException("Administrative Structure deletion failed", e))
            );
  }
