...
import ke.co.miles.accountabilitiestypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAccountabilitiesTypesQuery {
  ...
  /**
   * Selects the total number of accountabilities types records to be retrieved, or an estimate of
   ...
   *
   * @param database   the name of the database from which the total accountabilities types records
   *                   should be retrieved
  ...
  public Mono<Long> selectTotalAccountabilitiesTypes(String database,
    ...
    log.trace("Entering selectTotalAccountabilitiesTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM accountability_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM accountability_type"
        + new QueryWhereClauseBuilder().queryParameters(parameters).build();
  }
