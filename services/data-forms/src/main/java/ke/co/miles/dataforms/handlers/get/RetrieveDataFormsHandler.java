 ...
 */
package ke.co.miles.dataforms.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.dataforms.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveDataFormsHandler {

  @Autowired
  DataFormsRepository repository;

  /**
   * Retrieves dataForms records
   *
   * @param request the request, optionally containing the query filters of the dataForms
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved dataForms records
   */
  public Mono<ServerResponse> retrieveDataForms(ServerRequest request) {

    log.trace("Entering retrieveDataForms()");

    return
        repository
            .selectTotalDataForms(request.queryParams())
                    ...
                    .body(
                        repository
                            .selectDataForms(
                                request.queryParams()),
                        DataForm.class)
                    .onErrorMap(e -> new ServerException("Data Form deletion failed", e))
            );
  }
