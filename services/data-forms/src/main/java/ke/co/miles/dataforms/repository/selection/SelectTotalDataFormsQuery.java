...
import ke.co.miles.dataforms.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalDataFormsQuery {
  ...
  /**
   * Selects the total number of dataForms records to be retrieved, or an estimate of the total
   * number of dataForms records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of dataForms records to be retrieved
   */
  public Mono<Long> selectTotalDataForms(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalDataForms()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM data_form" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM data_form" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
