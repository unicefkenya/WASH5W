...
import ke.co.miles.systemsroles.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalSystemsRolesQuery {
  ...
  /**
   * Selects the total number of systemsRoles records to be retrieved, or an estimate of the total
   * number of systemsRoles records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of systemsRoles records to be retrieved
   */
  public Mono<Long> selectTotalSystemsRoles(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalSystemsRoles()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM system_role" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM system_role" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
