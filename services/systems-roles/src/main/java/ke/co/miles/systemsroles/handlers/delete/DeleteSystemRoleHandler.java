 ...
 */
package ke.co.miles.systemsroles.handlers.delete;
...
@Component
@Slf4j
public class DeleteSystemRoleHandler {
   ...
   *
   * @param request the request containing the details of the systemRole record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of systemsRoles records deleted
   */
  public Mono<ServerResponse> deleteSystemRole(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteSystemRoleById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
