...
import ke.co.miles.systemsroles.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveSystemsRolesHandler {

  @Autowired
  SystemsRolesRepository repository;

  /**
   * Retrieves systemsRoles records
   *
   * @param request the request, optionally containing the query filters of the systemsRoles
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved systemsRoles records
   */
  public Mono<ServerResponse> retrieveSystemsRoles(ServerRequest request) {

    log.trace("Entering retrieveSystemsRoles()");

    return
        repository
            .selectTotalSystemsRoles(request.queryParams())
