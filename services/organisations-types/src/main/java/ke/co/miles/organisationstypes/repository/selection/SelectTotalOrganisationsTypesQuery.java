...
import ke.co.miles.organisationstypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalOrganisationsTypesQuery {
  ...
  /**
   * Selects the total number of organisationsTypes records to be retrieved, or an estimate of the total
   * number of organisationsTypes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of organisationsTypes records to be retrieved
   */
  public Mono<Long> selectTotalOrganisationsTypes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalOrganisationsTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM organisation_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM organisation_type" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
