 ...
 */
package ke.co.miles.organisationstypes.handlers.delete;
...
@Component
@Slf4j
public class DeleteOrganisationTypeHandler {
   ...
   *
   * @param request the request containing the details of the organisationType record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of organisationsTypes records deleted
   */
  public Mono<ServerResponse> deleteOrganisationType(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteOrganisationTypeById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
