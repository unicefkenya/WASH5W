...
import ke.co.miles.organisationstypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveOrganisationsTypesHandler {

  @Autowired
  OrganisationsTypesRepository repository;

  /**
   * Retrieves organisationsTypes records
   *
   * @param request the request, optionally containing the query filters of the organisationsTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved organisationsTypes records
   */
  public Mono<ServerResponse> retrieveOrganisationsTypes(ServerRequest request) {

    log.trace("Entering retrieveOrganisationsTypes()");

    return
        repository
            .selectTotalOrganisationsTypes(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectOrganisationsTypes(
                                request.queryParams()),
                        OrganisationType.class)
                    .onErrorMap(e -> new ServerException("Organisation Type deletion failed", e))
            );
  }
