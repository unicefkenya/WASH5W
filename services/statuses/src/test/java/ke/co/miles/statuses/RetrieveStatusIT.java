...
@ContextConfiguration(initializers = RetrieveStatusIT.Initializer.class)
public class RetrieveStatusIT {
    ...
    @Test
    public void Given_StatusRecordExists_When_GetWithIdParameter_Then_TheStatusRecordWithThatIdWillBeReturned() {

        webTestClient
                .get()
                .uri(uriBuilder ->
                        uriBuilder
                                .path("/api/v1/statuses/test/ids/{id}")
                                .build(Long.toString(status1.getId())))
                .exchange()
                .expectStatus().isOk()
                ...
                .value(response -> {
                            ...
                            Assertions.assertThat(response.getVersion()).isEqualTo(status1.getVersion());
                        }
                );
    }
}
