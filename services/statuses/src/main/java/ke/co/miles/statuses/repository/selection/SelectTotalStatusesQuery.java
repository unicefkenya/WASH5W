...
import ke.co.miles.statuses.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalStatusesQuery {
    ...
    /**
     * Selects the total number of statuses records to be retrieved, or an estimate of the total number of
     * statuses records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total status records should be retrieved
    ...
    public Mono<Long> selectTotalStatuses(String database, MultiValueMap<String,String> parameters) {
        ...
        log.trace("Entering selectTotalStatuses()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM status" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM status" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
