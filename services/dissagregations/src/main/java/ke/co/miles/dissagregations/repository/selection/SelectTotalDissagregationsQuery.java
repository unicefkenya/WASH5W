...
import ke.co.miles.dissagregations.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalDissagregationsQuery {
  ...
  /**
   * Selects the total number of dissagregations records to be retrieved, or an estimate of the total
   * number of dissagregations records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of dissagregations records to be retrieved
   */
  public Mono<Long> selectTotalDissagregations(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalDissagregations()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM dissagregation" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM dissagregation" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
