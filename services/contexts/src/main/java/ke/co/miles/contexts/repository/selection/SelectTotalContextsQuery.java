...
import ke.co.miles.contexts.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalContextsQuery {
  ...
  /**
   * Selects the total number of contexts records to be retrieved, or an estimate of the total
   * number of contexts records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of contexts records to be retrieved
   */
  public Mono<Long> selectTotalContexts(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalContexts()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM context" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM context" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
