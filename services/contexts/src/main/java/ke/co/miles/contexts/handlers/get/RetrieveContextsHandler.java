...
import ke.co.miles.contexts.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveContextsHandler {

  @Autowired
  ContextsRepository repository;

  /**
   * Retrieves contexts records
   *
   * @param request the request, optionally containing the query filters of the contexts
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved contexts records
   */
  public Mono<ServerResponse> retrieveContexts(ServerRequest request) {

    log.trace("Entering retrieveContexts()");

    return
        repository
            .selectTotalContexts(request.queryParams())
