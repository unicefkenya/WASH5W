 ...
 */
package ke.co.miles.contexts.handlers.delete;
...
public class DeleteContextHandler {
   ...
   *
   * @param request the request containing the details of the context record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of contexts records deleted
   */
  public Mono<ServerResponse> deleteContext(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteContextById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
