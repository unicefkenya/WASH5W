...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION initializeRoleVersion()
RETURNS "trigger" AS '
BEGIN
  New.version:=1;
  RETURN NEW;
...
CREATE TRIGGER initializeRoleVersionTrigger
BEFORE INSERT
ON _role
FOR EACH ROW
EXECUTE PROCEDURE initializeRoleVersion();
...
CREATE OR REPLACE FUNCTION incrementRoleVersion()
RETURNS "trigger" AS
'
BEGIN
...
CREATE TRIGGER incrementRoleVersionTrigger
BEFORE UPDATE
ON _role
FOR EACH ROW
EXECUTE PROCEDURE incrementRoleVersion();
...
INSERT INTO _role(data) VALUES('{"index": 1, "name": "First Role", "valid": true}');
INSERT INTO _role(data) VALUES('{"index": 2, "name": "Second Role", "valid": true}');
INSERT INTO _role(data) VALUES('{"index": 3, "name": "Third Role", "valid": false}');