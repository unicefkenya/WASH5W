...
@Component
@Slf4j
public class SelectTotalRolesQuery {
    ...
    /**
     * Selects the total number of roles records to be retrieved, or an estimate of the total number of
     * roles records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total role records should be retrieved
     * @param parameters the query parameters passed along with the request
     *
     * @return the response containing the total number of roles records to be retrieved
     */
    public Mono<Long> selectTotalRoles(String database, MultiValueMap<String,String> parameters) {

        log.trace("Entering selectTotalRoles()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM _role" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM _role" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
