...
import ke.co.miles.options.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveOptionsHandler {

  @Autowired
  OptionsRepository repository;

  /**
   * Retrieves options records
   *
   * @param request the request, optionally containing the query filters of the options
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved options records
   */
  public Mono<ServerResponse> retrieveOptions(ServerRequest request) {

    log.trace("Entering retrieveOptions()");

    return
        repository
            .selectTotalOptions(request.queryParams())
