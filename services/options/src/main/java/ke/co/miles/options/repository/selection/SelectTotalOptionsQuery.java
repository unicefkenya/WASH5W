...
import ke.co.miles.options.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalOptionsQuery {
  ...
  /**
   * Selects the total number of options records to be retrieved, or an estimate of the total
   * number of options records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of options records to be retrieved
   */
  public Mono<Long> selectTotalOptions(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalOptions()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM option" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM option" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
