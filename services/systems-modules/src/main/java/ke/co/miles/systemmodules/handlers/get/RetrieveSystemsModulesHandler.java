...
import ke.co.miles.systemmodules.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveSystemsModulesHandler {

  @Autowired
  SystemsModulesRepository repository;

  /**
   * Retrieves systemsModules records
   *
   * @param request the request, optionally containing the query filters of the systemsModules
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved systemsModules records
   */
  public Mono<ServerResponse> retrieveSystemsModules(ServerRequest request) {

    log.trace("Entering retrieveSystemsModules()");

    return
        repository
            .selectTotalSystemsModules(request.queryParams())
