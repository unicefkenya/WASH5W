...
import ke.co.miles.administrativesystems.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveAdministrativeSystemsHandler {

  @Autowired
  AdministrativeSystemsRepository repository;

  /**
   * Retrieves administrativeSystems records
   *
   * @param request the request, optionally containing the query filters of the administrativeSystems
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved administrativeSystems records
   */
  public Mono<ServerResponse> retrieveAdministrativeSystems(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeSystems()");

    return
        repository
            .selectTotalAdministrativeSystems(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectAdministrativeSystems(
                                request.queryParams()),
                        AdministrativeSystem.class)
                    .onErrorMap(e -> new ServerException("Administrative System deletion failed", e))
            );
  }
