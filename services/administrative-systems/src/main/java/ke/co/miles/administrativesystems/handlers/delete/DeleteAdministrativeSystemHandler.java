 ...
 */
package ke.co.miles.administrativesystems.handlers.delete;
...
@Component
@Slf4j
public class DeleteAdministrativeSystemHandler {
   ...
   *
   * @param request the request containing the details of the administrativeSystem record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of administrativeSystems records deleted
   */
  public Mono<ServerResponse> deleteAdministrativeSystem(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteAdministrativeSystemById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
