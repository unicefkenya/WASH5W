...
import ke.co.miles.administrativesystems.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAdministrativeSystemsQuery {
  ...
  /**
   * Selects the total number of administrativeSystems records to be retrieved, or an estimate of the total
   * number of administrativeSystems records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of administrativeSystems records to be retrieved
   */
  public Mono<Long> selectTotalAdministrativeSystems(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAdministrativeSystems()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM administrative_system" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM administrative_system" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
