...
import ke.co.miles.logicalelements.daos.QueryEstimates;
import ke.co.miles.logicalelements.exceptions.ServerException;
import ke.co.miles.logicalelements.util.builders.QueryWhereClauseBuilder;
...
@Component
@Slf4j
public class SelectTotalLogicalElementsQuery {
  ...
  /**
   * Selects the total number of logicalElements records to be retrieved, or an estimate of the total
   * number of logicalElements records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of logicalElements records to be retrieved
   */
  public Mono<Long> selectTotalLogicalElements(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalLogicalElements()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM logical_element" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM logical_element" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
