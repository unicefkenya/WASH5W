...
import ke.co.miles.logicalelements.util.builders.QueryPaginationClauseBuilder;
...
@Component
@Slf4j
public class SelectLogicalElementsQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects all or specific logicalElements records from the database depending on whether query
   * parameters were supplied as part of the query
   *
   * @param parameters the query parameters passed along with the request
   * @return a list of logicalElements records if found
   */
  public Flux<LogicalElement> selectLogicalElements(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectLogicalElements()");

    String query =
        "SELECT * FROM logical_element" +
