...
import ke.co.miles.logicalelements.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveLogicalElementsHandler {

  @Autowired
  LogicalElementsRepository repository;

  /**
   * Retrieves logicalElements records
   *
   * @param request the request, optionally containing the query filters of the logicalElements
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved logicalElements records
   */
  public Mono<ServerResponse> retrieveLogicalElements(ServerRequest request) {

    log.trace("Entering retrieveLogicalElements()");

    return
        repository
            .selectTotalLogicalElements(request.queryParams())
