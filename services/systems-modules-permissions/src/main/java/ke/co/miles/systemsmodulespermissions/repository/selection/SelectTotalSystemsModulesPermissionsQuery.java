...
import ke.co.miles.systemsmodulespermissions.daos.QueryEstimates;
import ke.co.miles.systemsmodulespermissions.exceptions.ServerException;
import ke.co.miles.systemsmodulespermissions.util.builders.QueryWhereClauseBuilder;
...
@Component
@Slf4j
public class SelectTotalSystemsModulesPermissionsQuery {
  ...
  /**
   * Selects the total number of systemsModulesPermissions records to be retrieved, or an estimate of the total
   * number of systemsModulesPermissions records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of systemsModulesPermissions records to be retrieved
   */
  public Mono<Long> selectTotalSystemsModulesPermissions(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalSystemsModulesPermissions()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM system_module_permission" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM system_module_permission" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
