...
import ke.co.miles.systemsmodulespermissions.util.builders.QueryPaginationClauseBuilder;
...
@Component
@Slf4j
public class SelectSystemsModulesPermissionsQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects all or specific systemsModulesPermissions records from the database depending on whether query
   * parameters were supplied as part of the query
   *
   * @param parameters the query parameters passed along with the request
   * @return a list of systemsModulesPermissions records if found
   */
  public Flux<SystemModulePermission> selectSystemsModulesPermissions(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectSystemsModulesPermissions()");

    String query =
        "SELECT * FROM system_module_permission" +
