...
import ke.co.miles.systemsmodulespermissions.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveSystemsModulesPermissionsHandler {

  @Autowired
  SystemsModulesPermissionsRepository repository;

  /**
   * Retrieves systemsModulesPermissions records
   *
   * @param request the request, optionally containing the query filters of the systemsModulesPermissions
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved systemsModulesPermissions records
   */
  public Mono<ServerResponse> retrieveSystemsModulesPermissions(ServerRequest request) {

    log.trace("Entering retrieveSystemsModulesPermissions()");

    return
        repository
            .selectTotalSystemsModulesPermissions(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectSystemsModulesPermissions(
                                request.queryParams()),
