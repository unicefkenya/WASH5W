 ...
 */
package ke.co.miles.systemsmodulespermissions.handlers.delete;
...
@Component
@Slf4j
public class DeleteSystemModulePermissionHandler {
   ...
   *
   * @param request the request containing the details of the systemModulePermission record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of systemsModulesPermissions records deleted
   */
  public Mono<ServerResponse> deleteSystemModulePermission(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteSystemModulePermissionById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
