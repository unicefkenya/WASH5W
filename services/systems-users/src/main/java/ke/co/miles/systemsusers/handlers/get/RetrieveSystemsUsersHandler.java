...
import ke.co.miles.systemsusers.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveSystemsUsersHandler {

  @Autowired
  SystemsUsersRepository repository;

  /**
   * Retrieves systemsUsers records
   *
   * @param request the request, optionally containing the query filters of the systemsUsers
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved systemsUsers records
   */
  public Mono<ServerResponse> retrieveSystemsUsers(ServerRequest request) {

    log.trace("Entering retrieveSystemsUsers()");

    return
        repository
            .selectTotalSystemsUsers(request.queryParams())
                    ...
                    .body(
                        repository
                            .selectSystemsUsers(
                                request.queryParams()),
                        SystemUser.class)
                    .onErrorMap(e -> new ServerException("System User deletion failed", e))
            );
  }
