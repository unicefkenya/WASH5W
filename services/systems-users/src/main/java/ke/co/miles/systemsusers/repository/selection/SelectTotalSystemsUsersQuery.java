...
import ke.co.miles.systemsusers.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalSystemsUsersQuery {
  ...
  /**
   * Selects the total number of systemsUsers records to be retrieved, or an estimate of the total
   * number of systemsUsers records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of systemsUsers records to be retrieved
   */
  public Mono<Long> selectTotalSystemsUsers(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalSystemsUsers()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM system_user" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM system_user" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
