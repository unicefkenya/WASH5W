 ...
 */
package ke.co.miles.administrativeunitstypes.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.administrativeunitstypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveAdministrativeUnitsTypesHandler {

  @Autowired
  AdministrativeUnitsTypesRepository repository;

  /**
   * Retrieves administrativeUnitsTypes records
   *
   * @param request the request, optionally containing the query filters of the administrativeUnitsTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved administrativeUnitsTypes records
   */
  public Mono<ServerResponse> retrieveAdministrativeUnitsTypes(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeUnitsTypes()");

    return
        repository
            .selectTotalAdministrativeUnitsTypes(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectAdministrativeUnitsTypes(
                                request.queryParams()),
                        AdministrativeUnitType.class)
                    .onErrorMap(e -> new ServerException("Administrative Unit Type deletion failed", e))
            );
  }
