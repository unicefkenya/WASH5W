...
@Component
@Slf4j
public class RetrieveAdministrativeUnitTypeHandler {

  @Autowired
  AdministrativeUnitsTypesRepository repository;

  /**
   * Retrieves a administrativeUnitType record given its unique identifier
   *
   * @param request the request containing the unique identifier of the administrativeUnitType record to be
   *                retrieved and the database from which it should be retrieved
   * @return the response containing the details of the retrieved administrativeUnitType record
   */
  public Mono<ServerResponse> retrieveAdministrativeUnitType(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeUnitType()");
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .selectAdministrativeUnitType(Long.parseLong(request.pathVariable("id"))),
