...
import ke.co.miles.administrativeunitstypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAdministrativeUnitsTypesQuery {
  ...
  /**
   * Selects the total number of administrativeUnitsTypes records to be retrieved, or an estimate of the total
   * number of administrativeUnitsTypes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of administrativeUnitsTypes records to be retrieved
   */
  public Mono<Long> selectTotalAdministrativeUnitsTypes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAdministrativeUnitsTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM administrative_unit_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM administrative_unit_type" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
