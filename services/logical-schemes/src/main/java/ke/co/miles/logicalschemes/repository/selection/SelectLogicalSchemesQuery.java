...
import ke.co.miles.logicalschemes.util.builders.QueryPaginationClauseBuilder;
...
@Component
@Slf4j
public class SelectLogicalSchemesQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects all or specific logicalSchemes records from the database depending on whether query
   * parameters were supplied as part of the query
   *
   * @param parameters the query parameters passed along with the request
   * @return a list of logicalSchemes records if found
   */
  public Flux<LogicalScheme> selectLogicalSchemes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectLogicalSchemes()");

    String query =
        "SELECT * FROM logical_scheme" +
