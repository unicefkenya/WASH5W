...
import ke.co.miles.logicalschemes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveLogicalSchemesHandler {

  @Autowired
  LogicalSchemesRepository repository;

  /**
   * Retrieves logicalSchemes records
   *
   * @param request the request, optionally containing the query filters of the logicalSchemes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved logicalSchemes records
   */
  public Mono<ServerResponse> retrieveLogicalSchemes(ServerRequest request) {

    log.trace("Entering retrieveLogicalSchemes()");

    return
        repository
            .selectTotalLogicalSchemes(request.queryParams())
