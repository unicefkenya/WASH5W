 ...
 */
package ke.co.miles.logicalschemes.handlers.delete;
...
@Component
@Slf4j
public class DeleteLogicalSchemeHandler {
   ...
   *
   * @param request the request containing the details of the logicalScheme record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of logicalSchemes records deleted
   */
  public Mono<ServerResponse> deleteLogicalScheme(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteLogicalSchemeById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
