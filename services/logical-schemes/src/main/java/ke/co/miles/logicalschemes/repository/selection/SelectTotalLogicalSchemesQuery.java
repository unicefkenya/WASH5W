...
import ke.co.miles.logicalschemes.daos.QueryEstimates;
import ke.co.miles.logicalschemes.exceptions.ServerException;
import ke.co.miles.logicalschemes.util.builders.QueryWhereClauseBuilder;
...
@Component
@Slf4j
public class SelectTotalLogicalSchemesQuery {
  ...
  /**
   * Selects the total number of logicalSchemes records to be retrieved, or an estimate of the total
   * number of logicalSchemes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of logicalSchemes records to be retrieved
   */
  public Mono<Long> selectTotalLogicalSchemes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalLogicalSchemes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM logical_scheme" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM logical_scheme" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
