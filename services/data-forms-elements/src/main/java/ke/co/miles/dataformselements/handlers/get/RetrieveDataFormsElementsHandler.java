...
import ke.co.miles.dataformselements.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveDataFormsElementsHandler {

  @Autowired
  DataFormsElementsRepository repository;

  /**
   * Retrieves dataFormsElements records
   *
   * @param request the request, optionally containing the query filters of the dataFormsElements
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved dataFormsElements records
   */
  public Mono<ServerResponse> retrieveDataFormsElements(ServerRequest request) {

    log.trace("Entering retrieveDataFormsElements()");

    return
        repository
            .selectTotalDataFormsElements(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectDataFormsElements(
                                request.queryParams()),
                        DataFormElement.class)
                    .onErrorMap(e -> new ServerException("Data Form Element deletion failed", e))
            );
  }
