...
import ke.co.miles.dataformselements.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalDataFormsElementsQuery {
  ...
  /**
   * Selects the total number of dataFormsElements records to be retrieved, or an estimate of the total
   * number of dataFormsElements records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of dataFormsElements records to be retrieved
   */
  public Mono<Long> selectTotalDataFormsElements(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalDataFormsElements()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM data_form_element_" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM data_form_element_" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
