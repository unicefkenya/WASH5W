 ...
 */
package ke.co.miles.dataformselements.handlers.delete;
...
@Component
@Slf4j
public class DeleteDataFormElementHandler {
   ...
   *
   * @param request the request containing the details of the dataFormElement record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of dataFormsElements records deleted
   */
  public Mono<ServerResponse> deleteDataFormElement(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteDataFormElementById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
