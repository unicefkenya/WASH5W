 ...
 */
package ke.co.miles.organisations.handlers.delete;
...
@Component
@Slf4j
public class DeleteOrganisationHandler {
   ...
   *
   * @param request the request containing the details of the organisation record to be deleted and the
   *                database from which it should be deleted
   * @return the response containing the number of organisations records deleted
   */
  public Mono<ServerResponse> deleteOrganisation(ServerRequest request) {
    ...
    return
        ServerResponse
            ...
            .body(
                repository
                    .deleteOrganisationById(Long.parseLong(request.pathVariable("id"))),
                Integer.class)
