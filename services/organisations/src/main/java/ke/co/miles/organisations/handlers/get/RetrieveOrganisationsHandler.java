...
import ke.co.miles.organisations.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveOrganisationsHandler {

  @Autowired
  OrganisationsRepository repository;

  /**
   * Retrieves organisations records
   *
   * @param request the request, optionally containing the query filters of the organisations
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved organisations records
   */
  public Mono<ServerResponse> retrieveOrganisations(ServerRequest request) {

    log.trace("Entering retrieveOrganisations()");

    return
        repository
            .selectTotalOrganisations(request.queryParams())
