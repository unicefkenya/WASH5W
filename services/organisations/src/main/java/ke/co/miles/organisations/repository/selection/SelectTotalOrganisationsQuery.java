...
import ke.co.miles.organisations.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalOrganisationsQuery {
  ...
  /**
   * Selects the total number of organisations records to be retrieved, or an estimate of the total
   * number of organisations records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of organisations records to be retrieved
   */
  public Mono<Long> selectTotalOrganisations(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalOrganisations()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM organisation" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM organisation" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
