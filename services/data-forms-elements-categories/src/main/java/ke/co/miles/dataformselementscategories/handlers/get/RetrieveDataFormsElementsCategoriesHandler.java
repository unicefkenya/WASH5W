 ...
 */
package ke.co.miles.dataformselementscategories.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.dataformselementscategories.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveDataFormsElementsCategoriesHandler {

  @Autowired
  DataFormsElementsCategoriesRepository repository;

  /**
   * Retrieves dataFormsElementsCategories records
   *
   * @param request the request, optionally containing the query filters of the dataFormsElementsCategories
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved dataFormsElementsCategories records
   */
  public Mono<ServerResponse> retrieveDataFormsElementsCategories(ServerRequest request) {

    log.trace("Entering retrieveDataFormsElementsCategories()");

    return
        repository
            .selectTotalDataFormsElementsCategories(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectDataFormsElementsCategories(
                                request.queryParams()),
                        DataFormElementCategory.class)
                    .onErrorMap(e -> new ServerException("Data Form Element Category deletion failed", e))
            );
  }
