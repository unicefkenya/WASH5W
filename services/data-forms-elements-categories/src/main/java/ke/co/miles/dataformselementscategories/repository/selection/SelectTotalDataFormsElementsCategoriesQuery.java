...
import ke.co.miles.dataformselementscategories.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalDataFormsElementsCategoriesQuery {
  ...
  /**
   * Selects the total number of dataFormsElementsCategories records to be retrieved, or an estimate of the total
   * number of dataFormsElementsCategories records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of dataFormsElementsCategories records to be retrieved
   */
  public Mono<Long> selectTotalDataFormsElementsCategories(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalDataFormsElementsCategories()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM data_form_element_category" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM data_form_element_category" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
