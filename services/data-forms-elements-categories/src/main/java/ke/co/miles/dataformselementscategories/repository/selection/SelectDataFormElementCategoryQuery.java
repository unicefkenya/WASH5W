...
@Component
@Slf4j
public class SelectDataFormElementCategoryQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects a dataFormElementCategory record from the database given its unique identifier
   *
   * @param id the unique identifier of the dataFormElementCategory record to be selected
   * @return the dataFormElementCategory record with the given id if found
   */
  public Mono<DataFormElementCategory> selectDataFormElementCategory(Long id) {

    log.trace("Entering selectDataFormElementCategory()");

    String query = "SELECT * FROM data_form_element_category WHERE id = ?";
