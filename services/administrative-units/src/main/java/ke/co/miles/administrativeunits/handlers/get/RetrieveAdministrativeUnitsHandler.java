...
import ke.co.miles.administrativeunits.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveAdministrativeUnitsHandler {

  @Autowired
  AdministrativeUnitsRepository repository;

  /**
   * Retrieves administrativeUnits records
   *
   * @param request the request, optionally containing the query filters of the administrativeUnits
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved administrativeUnits records
   */
  public Mono<ServerResponse> retrieveAdministrativeUnits(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeUnits()");

    return
        repository
            .selectTotalAdministrativeUnits(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .onErrorMap(e -> new ServerException("Administrative Unit deletion failed", e))
            );
  }
