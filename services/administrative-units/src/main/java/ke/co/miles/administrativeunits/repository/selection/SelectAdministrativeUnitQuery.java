...
@Component
@Slf4j
public class SelectAdministrativeUnitQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects a administrativeUnit record from the database given its unique identifier
   *
   * @param id the unique identifier of the administrativeUnit record to be selected
   * @return the administrativeUnit record with the given id if found
   */
  public Mono<AdministrativeUnit> selectAdministrativeUnit(Long id) {

    log.trace("Entering selectAdministrativeUnit()");

    String query = "SELECT * FROM administrative_unit WHERE id = ?";
