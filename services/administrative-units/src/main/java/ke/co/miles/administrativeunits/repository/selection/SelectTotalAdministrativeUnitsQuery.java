...
import ke.co.miles.administrativeunits.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAdministrativeUnitsQuery {
  ...
  /**
   * Selects the total number of administrativeUnits records to be retrieved, or an estimate of the total
   * number of administrativeUnits records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of administrativeUnits records to be retrieved
   */
  public Mono<Long> selectTotalAdministrativeUnits(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAdministrativeUnits()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM administrative_unit" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM administrative_unit" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
