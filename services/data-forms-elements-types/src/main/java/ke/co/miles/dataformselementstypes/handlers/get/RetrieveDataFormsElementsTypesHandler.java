...
import ke.co.miles.dataformselementstypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveDataFormsElementsTypesHandler {

  @Autowired
  DataFormsElementsTypesRepository repository;

  /**
   * Retrieves dataFormsElementsTypes records
   *
   * @param request the request, optionally containing the query filters of the dataFormsElementsTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved dataFormsElementsTypes records
   */
  public Mono<ServerResponse> retrieveDataFormsElementsTypes(ServerRequest request) {

    log.trace("Entering retrieveDataFormsElementsTypes()");

    return
        repository
            .selectTotalDataFormsElementsTypes(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectDataFormsElementsTypes(
                                request.queryParams()),
                        DataFormElementType.class)
                    .onErrorMap(e -> new ServerException("Data Form Element Type deletion failed", e))
            );
  }
