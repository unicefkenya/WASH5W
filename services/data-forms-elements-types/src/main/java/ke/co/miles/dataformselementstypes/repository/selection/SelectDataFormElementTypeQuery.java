...
@Component
@Slf4j
public class SelectDataFormElementTypeQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects a dataFormElementType record from the database given its unique identifier
   *
   * @param id the unique identifier of the dataFormElementType record to be selected
   * @return the dataFormElementType record with the given id if found
   */
  public Mono<DataFormElementType> selectDataFormElementType(Long id) {

    log.trace("Entering selectDataFormElementType()");

    String query = "SELECT * FROM data_form_element_type WHERE id = ?";
