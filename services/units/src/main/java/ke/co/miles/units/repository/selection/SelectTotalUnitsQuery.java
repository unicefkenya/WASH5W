...
import ke.co.miles.units.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalUnitsQuery {
  ...
  /**
   * Selects the total number of units records to be retrieved, or an estimate of the total
   * number of units records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of units records to be retrieved
   */
  public Mono<Long> selectTotalUnits(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalUnits()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM unit" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM unit" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
