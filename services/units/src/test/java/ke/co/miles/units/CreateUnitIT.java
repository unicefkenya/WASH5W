...
public class CreateUnitIT {
  ...
  static {
    ...
    unit4 =
        new UnitBuilder()
            .id(null)
            .data(
                "{\"name\":\"Fourth Unit Name\",\"abbreviation\":\"FUN\"}")
            .version(null)
            .build();
  }
  ...
  public void Given_UnitDetails_When_Post_Then_UnitRecordWillBeCreatedAndReturned() {
    ...
    webTestClient
        ...
        .body(Mono.just(unit4), Unit.class)
        .exchange()
        .expectStatus().isCreated()
        .expectBody(Unit.class)
        .value(response -> {
              Assertions.assertThat(response.getId()).isEqualTo(4L);
              Assertions.assertThat(response.getData()).isEqualTo(unit4.getData());
