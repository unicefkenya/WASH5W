...
@ContextConfiguration(initializers = RetrieveUnitIT.Initializer.class)
public class RetrieveUnitIT {
    ...
    webTestClient
        ...
        .uri(uriBuilder ->
            uriBuilder
                ...
                .build(Long.toString(unit1.getId())))
        .exchange()
        .expectStatus().isOk()
        .expectBody(Unit.class)
        .value(response -> {
              Assertions.assertThat(response.getId()).isEqualTo(unit1.getId());
              try {
                JSONAssert.assertEquals(unit1.getData(), response.getData(), false);
              } catch (JSONException e) {
                throw new RuntimeException(e);
              }
              Assertions.assertThat(response.getVersion()).isEqualTo(unit1.getVersion());
            }
        );
  }
