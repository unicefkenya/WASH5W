...
import ke.co.miles.workflowstatuses.util.builders.QueryPaginationClauseBuilder;
...
@Component
@Slf4j
public class SelectWorkflowStatusesQuery {

  @Autowired
  DatabaseConfig databaseConfig;

  /**
   * Selects all or specific workflowStatuses records from the database depending on whether query
   * parameters were supplied as part of the query
   *
   * @param parameters the query parameters passed along with the request
   * @return a list of workflowStatuses records if found
   */
  public Flux<WorkflowStatus> selectWorkflowStatuses(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectWorkflowStatuses()");
