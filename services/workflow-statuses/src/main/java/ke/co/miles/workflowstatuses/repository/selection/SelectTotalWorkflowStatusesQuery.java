...
import ke.co.miles.workflowstatuses.daos.QueryEstimates;
import ke.co.miles.workflowstatuses.exceptions.ServerException;
import ke.co.miles.workflowstatuses.util.builders.QueryWhereClauseBuilder;
...
@Component
@Slf4j
public class SelectTotalWorkflowStatusesQuery {
  ...
  /**
   * Selects the total number of workflowStatuses records to be retrieved, or an estimate of the total
   * number of workflowStatuses records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of workflowStatuses records to be retrieved
   */
  public Mono<Long> selectTotalWorkflowStatuses(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalWorkflowStatuses()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM workflow_status" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM workflow_status" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
