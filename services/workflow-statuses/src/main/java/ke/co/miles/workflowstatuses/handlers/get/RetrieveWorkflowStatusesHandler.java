...
import ke.co.miles.workflowstatuses.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveWorkflowStatusesHandler {

  @Autowired
  WorkflowStatusesRepository repository;

  /**
   * Retrieves workflowStatuses records
   *
   * @param request the request, optionally containing the query filters of the workflowStatuses
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved workflowStatuses records
   */
  public Mono<ServerResponse> retrieveWorkflowStatuses(ServerRequest request) {

    log.trace("Entering retrieveWorkflowStatuses()");

    return
        repository
            .selectTotalWorkflowStatuses(request.queryParams())
