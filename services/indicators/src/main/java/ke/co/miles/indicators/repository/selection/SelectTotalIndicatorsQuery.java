...
import ke.co.miles.indicators.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalIndicatorsQuery {
  ...
  /**
   * Selects the total number of indicators records to be retrieved, or an estimate of the total
   * number of indicators records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of indicators records to be retrieved
   */
  public Mono<Long> selectTotalIndicators(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalIndicators()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM indicator" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM indicator" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
