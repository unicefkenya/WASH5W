...
import ke.co.miles.indicators.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveIndicatorsHandler {

  @Autowired
  IndicatorsRepository repository;

  /**
   * Retrieves indicators records
   *
   * @param request the request, indicatorally containing the query filters of the indicators
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved indicators records
   */
  public Mono<ServerResponse> retrieveIndicators(ServerRequest request) {

    log.trace("Entering retrieveIndicators()");

    return
        repository
            .selectTotalIndicators(request.queryParams())
