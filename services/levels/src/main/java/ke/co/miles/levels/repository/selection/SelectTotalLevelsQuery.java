...
import ke.co.miles.levels.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalLevelsQuery {
    ...
    /**
     * Selects the total number of levels records to be retrieved, or an estimate of the total number of
     * levels records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total level records should be retrieved
    ...
    public Mono<Long> selectTotalLevels(String database, MultiValueMap<String,String> parameters) {
        ...
        log.trace("Entering selectTotalLevels()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM level" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM level" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
