...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION initializeLevelVersion()
RETURNS "trigger" AS '
BEGIN
  New.version:=1;
  RETURN NEW;
...
CREATE TRIGGER initializeLevelVersionTrigger
BEFORE INSERT
ON level
FOR EACH ROW
EXECUTE PROCEDURE initializeLevelVersion();
...
CREATE OR REPLACE FUNCTION incrementLevelVersion()
RETURNS "trigger" AS
'
BEGIN
...
CREATE TRIGGER incrementLevelVersionTrigger
BEFORE UPDATE
ON level
FOR EACH ROW
EXECUTE PROCEDURE incrementLevelVersion();
...
INSERT INTO level(data) VALUES('{"index": 1, "name": "First Level", "valid": true}');
INSERT INTO level(data) VALUES('{"index": 2, "name": "Second Level", "valid": true}');
INSERT INTO level(data) VALUES('{"index": 3, "name": "Third Level", "valid": false}');