...
import ke.co.miles.logicalelementstypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveLogicalElementsTypesHandler {

  @Autowired
  LogicalElementsTypesRepository repository;

  /**
   * Retrieves logicalElementsTypes records
   *
   * @param request the request, optionally containing the query filters of the logicalElementsTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved logicalElementsTypes records
   */
  public Mono<ServerResponse> retrieveLogicalElementsTypes(ServerRequest request) {

    log.trace("Entering retrieveLogicalElementsTypes()");

    return
        repository
            .selectTotalLogicalElementsTypes(request.queryParams())
