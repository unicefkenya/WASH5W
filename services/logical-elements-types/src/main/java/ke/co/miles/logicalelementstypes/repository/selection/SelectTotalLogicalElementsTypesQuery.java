...
import ke.co.miles.logicalelementstypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalLogicalElementsTypesQuery {
  ...
  /**
   * Selects the total number of logicalElementsTypes records to be retrieved, or an estimate of the total
   * number of logicalElementsTypes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of logicalElementsTypes records to be retrieved
   */
  public Mono<Long> selectTotalLogicalElementsTypes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalLogicalElementsTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM logical_element_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM logical_element_type" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
