...
import ke.co.miles.optionstypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalOptionsTypesQuery {
  ...
  /**
   * Selects the total number of optionsTypes records to be retrieved, or an estimate of the total
   * number of optionsTypes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of optionsTypes records to be retrieved
   */
  public Mono<Long> selectTotalOptionsTypes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalOptionsTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM option_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM option_type" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
