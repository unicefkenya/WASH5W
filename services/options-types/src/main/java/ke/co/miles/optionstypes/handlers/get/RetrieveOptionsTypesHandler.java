...
import ke.co.miles.optionstypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveOptionsTypesHandler {

  @Autowired
  OptionsTypesRepository repository;

  /**
   * Retrieves optionsTypes records
   *
   * @param request the request, optionally containing the query filters of the optionsTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved optionsTypes records
   */
  public Mono<ServerResponse> retrieveOptionsTypes(ServerRequest request) {

    log.trace("Entering retrieveOptionsTypes()");

    return
        repository
            .selectTotalOptionsTypes(request.queryParams())
                    ...
                    .body(
                        repository
                            .selectOptionsTypes(
                                request.queryParams()),
                        OptionType.class)
                    .onErrorMap(e -> new ServerException("Option Type deletion failed", e))
            );
  }
