...
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION initializeTypeVersion()
RETURNS "trigger" AS '
BEGIN
  New.version:=1;
  RETURN NEW;
...
CREATE TRIGGER initializeTypeVersionTrigger
BEFORE INSERT
ON type
FOR EACH ROW
EXECUTE PROCEDURE initializeTypeVersion();
...
CREATE OR REPLACE FUNCTION incrementTypeVersion()
RETURNS "trigger" AS
'
BEGIN
...
CREATE TRIGGER incrementTypeVersionTrigger
BEFORE UPDATE
ON type
FOR EACH ROW
EXECUTE PROCEDURE incrementTypeVersion();
...
INSERT INTO type(data) VALUES('{"index": 1, "name": "First Type", "valid": true}');
INSERT INTO type(data) VALUES('{"index": 2, "name": "Second Type", "valid": true}');
INSERT INTO type(data) VALUES('{"index": 3, "name": "Third Type", "valid": false}');