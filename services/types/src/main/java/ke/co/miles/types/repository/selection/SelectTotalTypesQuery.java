...
import ke.co.miles.types.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalTypesQuery {
    ...
    /**
     * Selects the total number of types records to be retrieved, or an estimate of the total number of
     * types records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total type records should be retrieved
    ...
    public Mono<Long> selectTotalTypes(String database, MultiValueMap<String,String> parameters) {
        ...
        log.trace("Entering selectTotalTypes()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM type" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM type" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
