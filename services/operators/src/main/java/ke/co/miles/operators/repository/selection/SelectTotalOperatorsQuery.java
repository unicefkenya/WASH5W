...
import ke.co.miles.operators.daos.QueryEstimates;
import ke.co.miles.operators.exceptions.ServerException;
import ke.co.miles.operators.util.builders.QueryWhereClauseBuilder;
...
@Component
@Slf4j
public class SelectTotalOperatorsQuery {
  ...
  /**
   * Selects the total number of operators records to be retrieved, or an estimate of the total
   * number of operators records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of operators records to be retrieved
   */
  public Mono<Long> selectTotalOperators(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalOperators()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM operator" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM operator" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
