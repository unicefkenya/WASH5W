...
import ke.co.miles.operators.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveOperatorsHandler {

  @Autowired
  OperatorsRepository repository;

  /**
   * Retrieves operators records
   *
   * @param request the request, operatorally containing the query filters of the operators
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved operators records
   */
  public Mono<ServerResponse> retrieveOperators(ServerRequest request) {

    log.trace("Entering retrieveOperators()");

    return
        repository
            .selectTotalOperators(request.queryParams())
