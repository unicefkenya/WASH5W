...
import ke.co.miles.entities.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalEntitiesQuery {
  ...
  /**
   * Selects the total number of entities records to be retrieved, or an estimate of the total
   * number of entities records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param database   the name of the database from which the total entity records should be
   *                   retrieved
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of entities records to be retrieved
   */
  public Mono<Long> selectTotalEntities(String database, MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalEntities()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM entity" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM entity" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
