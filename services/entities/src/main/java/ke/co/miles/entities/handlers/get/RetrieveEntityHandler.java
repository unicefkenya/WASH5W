...
@Component
@Slf4j
public class RetrieveEntityHandler {

  @Autowired
  EntitiesRepository repository;

  /**
   * Retrieves an entity record given its unique identifier
   *
   * @param request the request containing the unique identifier of the entity record to be
   *                retrieved and the database from which it should be retrieved
   * @return the response containing the details of the retrieved entity record
   */
  public Mono<ServerResponse> retrieveEntity(ServerRequest request) {

    log.trace("Entering retrieveEntity()");
