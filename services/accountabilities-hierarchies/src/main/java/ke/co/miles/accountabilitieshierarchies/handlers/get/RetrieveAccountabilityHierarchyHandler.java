...
@Component
@Slf4j
public class RetrieveAccountabilityHierarchyHandler {

    @Autowired
    AccountabilitiesHierarchiesRepository repository;

    /**
     * Retrieves an accountability hierarchy record given its unique identifier
     *
     * @param request the request containing the unique identifier of the accountability hierarchy record to be retrieved and the
     *                database from which it should be retrieved
     * @return the response containing the details of the retrieved accountability hierarchy record
     */
    public Mono<ServerResponse> retrieveAccountabilityHierarchy(ServerRequest request) {

        log.trace("Entering retrieveAccountabilityHierarchy()");
