...
import ke.co.miles.accountabilitieshierarchies.util.builders.QueryPaginationClauseBuilder;
...
@Component
@Slf4j
public class SelectAccountabilitiesHierarchiesQuery {

    @Autowired
    DatabaseConfig databaseConfig;

    /**
     * Selects all or specific accountabilities hierarchies records from the database depending on whether
     * query parameters were supplied as part of the query
     *
     * @param database   the name of the database from which the accountabilities hierarchies records should be selected
     * @param parameters the query parameters passed along with the request
     * @return a list of accountabilities hierarchies records if found
     */
    public Flux<AccountabilityHierarchy> selectAccountabilitiesHierarchies(String database, MultiValueMap<String, String> parameters) {
        ...
        log.trace("Entering selectAccountabilitiesHierarchies()");

        String query =
                "SELECT * FROM accountability_hierarchy" +
