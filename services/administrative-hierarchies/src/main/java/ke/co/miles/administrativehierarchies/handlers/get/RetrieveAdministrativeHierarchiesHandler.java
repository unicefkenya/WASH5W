 ...
 */
package ke.co.miles.administrativehierarchies.handlers.get;

import java.util.ArrayList;
import java.util.List;
import ke.co.miles.administrativehierarchies.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveAdministrativeHierarchiesHandler {

  @Autowired
  AdministrativeHierarchiesRepository repository;

  /**
   * Retrieves administrativeHierarchies records
   *
   * @param request the request, optionally containing the query filters of the administrativeHierarchies
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved administrativeHierarchies records
   */
  public Mono<ServerResponse> retrieveAdministrativeHierarchies(ServerRequest request) {

    log.trace("Entering retrieveAdministrativeHierarchies()");

    return
        repository
            .selectTotalAdministrativeHierarchies(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectAdministrativeHierarchies(
                                request.queryParams()),
                        AdministrativeHierarchy.class)
                    .onErrorMap(e -> new ServerException("Administrative Hierarchy deletion failed", e))
            );
  }
