...
import ke.co.miles.administrativehierarchies.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAdministrativeHierarchiesQuery {
  ...
  /**
   * Selects the total number of administrativeHierarchies records to be retrieved, or an estimate of the total
   * number of administrativeHierarchies records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of administrativeHierarchies records to be retrieved
   */
  public Mono<Long> selectTotalAdministrativeHierarchies(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAdministrativeHierarchies()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM administrative_hierarchy" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM administrative_hierarchy" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
