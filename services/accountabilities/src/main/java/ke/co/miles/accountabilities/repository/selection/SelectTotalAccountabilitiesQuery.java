...
import ke.co.miles.accountabilities.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAccountabilitiesQuery {
  ...
  /**
   * Selects the total number of accountabilities records to be retrieved, or an estimate of
   * the total number of accountabilities records to be retrieved - if the cost of retrieving
   * the former is greater than the acceptable threshold
   *
   * @param database   the name of the database from which the total accountabilities records
   *                   should be retrieved
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of accountabilities records to be
   * retrieved
   */
  public Mono<Long> selectTotalAccountabilities(String database,
      MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalAccountabilities()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM accountability" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM accountability"
        + new QueryWhereClauseBuilder().queryParameters(parameters).build();
  }
