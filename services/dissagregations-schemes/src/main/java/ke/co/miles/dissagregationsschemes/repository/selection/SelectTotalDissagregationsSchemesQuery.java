...
import ke.co.miles.dissagregationsschemes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalDissagregationsSchemesQuery {
  ...
  /**
   * Selects the total number of dissagregationsSchemes records to be retrieved, or an estimate of the total
   * number of dissagregationsSchemes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of dissagregationsSchemes records to be retrieved
   */
  public Mono<Long> selectTotalDissagregationsSchemes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalDissagregationsSchemes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM dissagregation_scheme" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM dissagregation_scheme" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
