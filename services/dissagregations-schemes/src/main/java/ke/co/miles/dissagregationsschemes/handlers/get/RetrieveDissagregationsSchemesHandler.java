...
import ke.co.miles.dissagregationsschemes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveDissagregationsSchemesHandler {

  @Autowired
  DissagregationsSchemesRepository repository;

  /**
   * Retrieves dissagregationsSchemes records
   *
   * @param request the request, optionally containing the query filters of the dissagregationsSchemes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved dissagregationsSchemes records
   */
  public Mono<ServerResponse> retrieveDissagregationsSchemes(ServerRequest request) {

    log.trace("Entering retrieveDissagregationsSchemes()");

    return
        repository
            .selectTotalDissagregationsSchemes(request.queryParams())
            ...
            .flatMap(metadata ->
                ServerResponse
                    ...
                    .body(
                        repository
                            .selectDissagregationsSchemes(
                                request.queryParams()),
                        DissagregationScheme.class)
                    .onErrorMap(e -> new ServerException("Dissagregation Scheme deletion failed", e))
            );
  }
