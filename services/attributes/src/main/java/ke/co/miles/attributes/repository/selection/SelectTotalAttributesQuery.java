...
import ke.co.miles.attributes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalAttributesQuery {
    ...
    /**
     * Selects the total number of attributes records to be retrieved, or an estimate of the total number of
     * attributes records to be retrieved - if the cost of retrieving the former is greater than the acceptable
     * threshold
     *
     * @param database the name of the database from which the total attribute records should be retrieved
     * @param parameters the query parameters passed along with the request
     *
     * @return the response containing the total number of attributes records to be retrieved
     */
    public Mono<Long> selectTotalAttributes(String database, MultiValueMap<String,String> parameters) {

        log.trace("Entering selectTotalAttributes()");
    ...
    private String getEstimatesQuery(MultiValueMap<String,String> parameters) {
        return "SELECT getQueryEstimates($$" + " SELECT * FROM attribute" +
    ...
    private String getQuery(MultiValueMap<String,String> parameters) {
        return "SELECT COUNT(*) FROM attribute" + new QueryWhereClauseBuilder().queryParameters(parameters).build();
    }
