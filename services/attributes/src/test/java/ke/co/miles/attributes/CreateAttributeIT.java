...
@ContextConfiguration(initializers = CreateAttributeIT.Initializer.class)
public class CreateAttributeIT {
    ...
    @Test
    public void Given_AttributeDetails_When_Post_Then_AttributeRecordWillBeCreatedAndReturned() {

        webTestClient
                .post()
                .uri("/api/v1/attributes/test")
                .contentType(MediaType.APPLICATION_JSON)
                .body(Mono.just(attribute4), Attribute.class)
                .exchange()
                .expectStatus().isCreated()
                .expectBody(Attribute.class)
                .value(response -> {
                            Assertions.assertThat(response.getId()).isEqualTo(4L);
                            Assertions.assertThat(response.getData()).isEqualTo(attribute4.getData());
