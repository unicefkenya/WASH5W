...
@ContextConfiguration(initializers = UpdateEntityTypeIT.Initializer.class)
public class UpdateEntityTypeIT {
  ...
  public void Given_ModifiedDetailsOfAnExistingRecord_When_Put_Then_TheRecordWillBeUpdatedAndReturnedWithItsVersionIncrementedByOne() {
    ...
    webTestClient
        .put()
        .uri("/api/v1/entities_types/1")
        .contentType(MediaType.APPLICATION_JSON)
        .body(Mono.just(entityType1), EntityType.class)
        .exchange()
        .expectStatus()
        .isOk()
        .expectBody(EntityType.class)
        .value(response -> {
              Assertions.assertThat(response.getId()).isEqualTo(entityType1.getId());
              try {
                JSONAssert.assertEquals(entityType1.getData(), response.getData(), false);
              } catch (JSONException e) {
                throw new RuntimeException(e);
              }
              Assertions.assertThat(response.getVersion()).isEqualTo(entityType1.getVersion() + 1);
            }
        );
  }
