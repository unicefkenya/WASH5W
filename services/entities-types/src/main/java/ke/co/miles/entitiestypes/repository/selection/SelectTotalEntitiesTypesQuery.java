...
import ke.co.miles.entitiestypes.daos.QueryEstimates;
...
@Component
@Slf4j
public class SelectTotalEntitiesTypesQuery {
  ...
  /**
   * Selects the total number of entitiesTypes records to be retrieved, or an estimate of the total
   * number of entitiesTypes records to be retrieved - if the cost of retrieving the former is greater
   * than the acceptable threshold
   *
   * @param parameters the query parameters passed along with the request
   * @return the response containing the total number of entitiesTypes records to be retrieved
   */
  public Mono<Long> selectTotalEntitiesTypes(MultiValueMap<String, String> parameters) {

    log.trace("Entering selectTotalEntitiesTypes()");
  ...
  private String getEstimatesQuery(MultiValueMap<String, String> parameters) {
    return "SELECT getQueryEstimates($$" + " SELECT * FROM entity_type" +
  ...
  private String getQuery(MultiValueMap<String, String> parameters) {
    return "SELECT COUNT(*) FROM entity_type" + new QueryWhereClauseBuilder().queryParameters(parameters)
        .build();
  }
