...
import ke.co.miles.entitiestypes.daos.Metadata;
...
@Component
@Slf4j
public class RetrieveEntitiesTypesHandler {

  @Autowired
  EntitiesTypesRepository repository;

  /**
   * Retrieves entitiesTypes records
   *
   * @param request the request, optionally containing the query filters of the entitiesTypes
   *                records to be retrieved and the database from which they should be retrieved
   * @return the stream of responses containing the details of the retrieved entitiesTypes records
   */
  public Mono<ServerResponse> retrieveEntitiesTypes(ServerRequest request) {

    log.trace("Entering retrieveEntitiesTypes()");

    return
        repository
            .selectTotalEntitiesTypes(request.queryParams())
